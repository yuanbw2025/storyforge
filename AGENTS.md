...
- 短篇、小说转剧本、小说转漫画、漫剧前期生产分别是独立端到端产品。
- 漫剧前期生产在同一用户流程内承接一句话或已有小说，输出系列圣经、逐集剧本、版本化物料、分镜、参考帧与图片/视频生成提示词包；它复用小说能力但不接管小说 owner，终点停在外部视频生成之前。
- 短篇也可由作者显式派生世界；小说转剧本、小说转漫画、漫剧前期生产当前不提供“封装为世界”路径。
- 世界引擎只保存可版本化语义内容，不拥有产品媒资或上层运行状态。
- 跑团、角色聊天、AI 小镇和文字游戏只读引用冻结的世界版本；各自拥有生产、媒资、发布、运行与私域演化，运行结果不得自动回写世界引擎。
- 世界衍生产品只走 `S1 世界封存 → S2 产品定向 → S3 产品执行`；S2 在具体产品内完成配置、定向和明确开始，S3 才可生产、验收、发布、运行与私域演化。世界页不得直达正式 runtime。
...
- `Project` 只是 LocalWorkspace 壳：只保存稳定身份、用途、活动 World/Work 指针和工作区级开关；作品标题、简介、流派、状态、字数、文风和活动叙事计划只允许存在于 `Work`。禁止镜像、双写、回退读取和导入时展开旧 Project 对象。

开始功能前先声明：产品、S1/S2/S3 位置（独立产品声明自身阶段）、数据 owner、交接物、世界引用和媒资 owner。边界不清时先补产品契约，不用代码制造既成事实。
...
- `S2 产品定向`的设置属于具体产品，不能建立全产品万能配置表；`S3 产品执行`的正式生产必须有用户开始授权、产品 Brief 和冻结 SourcePlan，正式运行必须绑定不可变 ProductRelease。
