...
const developmentPage = await readFile(resolve(repositoryRoot, 'src/pages/TextGameDevelopmentPage.tsx'), 'utf8')
...
if (!developmentPage.includes("productId: openWorld ? 'upper.text-open-world'")
  || !developmentPage.includes('<ProductProductionStudio')) {
  throw new Error('现行文字游戏产品页没有接入文字开放世界专属 Creator')
}
