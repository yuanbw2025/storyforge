...
## v3.9.0-labs.1 — 2026-07-16 · 双提示词应用路线对比实验版

> 这是一次性实验版本，用来比较两种不同的提示词应用方式：① 原有业务页面/工作流使用的“激活模板”路线；② 新增“小说创作库”使用的独立 Prompt 资产运行路线。两条路线在本版本中并列存在，互不抢占模板配置。后续正式版本将移除第二条独立路线；本 Release 保留该实验代码，供参与测试的用户下载和反馈。

- 小说创作 Prompt 资产库完成生产安全加固：章节生成结果绑定生成时章节/章纲/世界，流式或中止结果不可采纳；正文写入前创建项目快照，拒绝冲突报告，并统一刷新字数、事实状态、检索块和层级摘要。
- Prompt 资产的自动资料改为一次性统一上下文注入，消除同一来源在多个语义变量中的重复复制；最终请求会把固定模板、作者手输和模型真实窗口一并计入预算，超限时在调用 API 前阻断。
- 小说创作 Prompt 按生成、提取、分析、审查四类任务路由到已配置模型；118 个资产改为进入资产库时按需加载，未变化的系统模板不再每次启动重写，首屏包体积继续满足质量预算。
