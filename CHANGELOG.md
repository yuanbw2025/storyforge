...
- **新增 PIPELINE-1 发送前提示词预览**：设置页与正文工具栏可开启「发送前预览」（默认关）；正文生成/续写在 `ai.start` 前可查看并编辑最终拼接的 System/User 提示词，改动只影响这一次发送、不写回模板。
- **重做冷战氛围题材包**：定位改为「近现代科技异世界」写作辅助（阵营对峙 / 信息延迟 / 体制摩擦），弱化地球冷战史复刻与情绪化叙事指令；补齐项目流派与题材包切换入口，近现代科技上限参考写入题材约束注入。
- **修复全局协调接线**：创作规则面板补传 `worldGroupId`；设定对齐按当前激活世界装配；角色生成复用 `CROSS_SETTING_SOURCE_KEYS`；设定对齐支持「起草修复稿」（不自动写回）。
- **新增全局设定协调机制**：设定生成面板新增 🎯仅本面板 / 🌐全局协调 切换开关，默认开启全局协调模式，经 `assembleCrossSettingContext` 读取跨设定源，减轻设定互不相通问题（仍为 LLM 软约束，非 CONSISTENCY-3 硬校验）。
- **新增设定对齐工具**：侧边栏「设置区」→「设定对齐」，一键用 AI 扫描全部已有设定，检测时间线/实力/地理/规则/关系/数量/存在性七类矛盾，逐条展示左右原文对照、矛盾原因和修复建议。
