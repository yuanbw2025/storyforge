/** Original authored roadshow script, recovered from 618d388e. No database or runtime ownership. */
import type { NarrativeBeatKind, NarrativeNodeKind } from '../../lib/types'
...
export const MIST_HARBOR_ROADSHOW_NODES: MistHarborRoadshowNode[] = [
  ...
  { key: 'father-log', kind: 'scene', title: '父亲留下的第七码', summary: '旧铜灯播放林泊川的最后记录：他既是事故参与者，也是阻止第二次共振的人。', successors: ['bell'] },
...
export const MIST_HARBOR_ROADSHOW_BEATS: MistHarborRoadshowBeat[] = [
  ...
  ...scene('vault', [
    ...
    ['narration', null, '校准带末端有林泊川的手写字：如果城市只能靠遗忘代价继续运转，这台机器就不配被称作守护。'],
  ...
  ...scene('north-tower', [
    ...
    ['dialogue', '余砚', '林泊川把私人记录和法律证据放在一起，却设计成只能先取出一个。'],
  ...
  ...scene('father-log', [
    ['narration', null, '蜡筒开始转动。林泊川的声音被十年盐雾磨得沙哑，却仍保持守灯人报时的平稳。'],
    ['dialogue', '林澈', '父亲，我不是来问你是不是英雄。我只想知道钟楼里还有什么选择。'],
    ['narration', null, '录音承认他参与设计了记忆校准层，也承认议会曾以“可逆遗忘”为条件批准试验。事故发生后，所谓可逆从未被验证。'],
    ['narration', null, '林泊川留下三种主钥匙位置：公开记录会让全港同时恢复记忆；限制共振能先救港但延迟真相；彻底断钟则必须引导船队离港。'],
  ...
  ...scene('council-seal', [
    ...
    ['narration', null, '他们没有听见林泊川最后说过什么，却取得了让活人承担责任的权力。'],
  ]),
  ...scene('bell', [
...
export const MIST_HARBOR_ROADSHOW_CHOICES: MistHarborRoadshowChoice[] = [
  ...
  { sourceNodeKey: 'father-log', choiceKey: 'father.bell', text: '带着不能原谅的真相前往钟楼', description: '不把林泊川简化成英雄或罪人。', targetNodeKey: 'bell' },
