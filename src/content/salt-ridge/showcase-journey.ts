...
} from '../../lib/open-world/runtime-package-production'
import { parseTextOpenWorldRuntimePackageV1 } from '../../lib/open-world/runtime-package'
...
import { createSaltRidgeShowcaseBasePackageV1 } from './showcase-package'
...
export interface SaltRidgeShowcaseJourneyV1 {
...
/** Historical type name retained for regression compatibility. */
export type TextOpenWorldPlayerJourneyFixtureV1 = SaltRidgeShowcaseJourneyV1
...
function upgradeQuestLifecycleToV18(actions: any): void {
  ...
  if (!stagedAbandon) throw new Error('盐脊展示包缺少普通任务放弃 Action')
...
function upgradeStructuredCombatTriplet(runtimePackage: TextOpenWorldRuntimePackageV1): void {
  ...
  progression.skills = progression.skills.map((skill: any) => {
    ...
    if (kind !== 'attack') throw new Error(`盐脊展示包遇到未定义技能机制:${String(kind)}`)
  ...
  if (!enemy) throw new Error('盐脊展示包缺少指定敌人')
...
function upgradeDirectorToV3(runtimePackage: TextOpenWorldRuntimePackageV1): void {
  ...
  director.randomEvents = director.randomEvents.map((event: any) => {
    ...
    if (!scene) throw new Error(`盐脊展示包缺少随机事件场景:${String(event.key)}`)
...
function bindMainObjectiveCompletion(runtimePackage: TextOpenWorldRuntimePackageV1): void {
  ...
  if (!scene || scene.sourceKind !== 'quest-objective' || scene.questKey !== keys.finalQuestKey) {
    throw new Error('盐脊展示包缺少最终主线目标场景')
  }
...
function addTwoEndingRoutes(runtimePackage: TextOpenWorldRuntimePackageV1): void {
  ...
  if (!resolutionScene) throw new Error('盐脊展示包缺少唯一最终主线收束场景')
  ...
  for (const spec of routeSpecs) {
    ...
    if (!ending) throw new Error(`盐脊展示包缺少结局定义:${spec.endingKey}`)
...
async function sealInputBindings(actions: any): Promise<void> {
  ...
  for (const binding of actions.inputBindings.actions) {
    ...
    if (!action) throw new Error(`盐脊展示包 InputBinding 引用未知 Action:${String(binding.actionKey)}`)
...
/**
 * Frozen, parser-verified Release payload for the built-in Salt Ridge journey.
 * It starts from the authored base package, then closes every contract required
 * by the current fresh Action v18 runtime.
 */
export async function createSaltRidgeShowcaseJourneyV1(): Promise<SaltRidgeShowcaseJourneyV1> {
  const runtimePackage = createSaltRidgeShowcaseBasePackageV1()
  const actions = runtimePackage.modules.actions.payload as any
...
/** Historical function name retained for regression compatibility. */
export const createTextOpenWorldPlayerJourneyFixtureV1 = createSaltRidgeShowcaseJourneyV1
