...
import {
  ...
  Download, GitBranch, Lock, PackageCheck, Play, Plus, RefreshCw, Rocket, ShieldCheck, Sparkles,
  Square, Unlock, Upload,
} from 'lucide-react'
import {
...
export function visualReviewRepairTargets(artifact: ProductProductionReviewArtifactV1 | undefined): Array<{
  ...
  return reviews.flatMap(value => {
    ...
    if (typeof row.artifactKey !== 'string' || !Array.isArray(row.issues)) return []
    ...
    return verdict === 'revise' || verdict === 'replace' || verdict === 'human-review'
      || issues.some(issue => issue.severity === 'blocking')
      ? [{ artifactKey: row.artifactKey, verdict, issues }]
      : []
  })
}
import {
...
import TextOpenWorldCreatorReleaseStudio from '../text-game/TextOpenWorldCreatorReleaseStudio'
...
function reviewArtifactLabel(key: string): string {
  ...
  return ({
    ...
    'media.repair-feedback': 'Visual QA 逐图返修证据',
...
function budgetLedgerHasOutstandingReservations(value: string | undefined): boolean {
  ...
  try {
    const parsed = JSON.parse(value) as { version?: unknown; attempts?: unknown }
    if (parsed.version !== 2 || !Array.isArray(parsed.attempts)) return true
    return parsed.attempts.some(attempt => {
      if (!attempt || typeof attempt !== 'object' || Array.isArray(attempt)) return true
      const row = attempt as Record<string, unknown>
      return row.usageKnown === false && row.usage != null
    })
  } catch { return true }
}
...
export default function ProductProductionStudio(props: {
  ...
  /** Open an already-created Production instead of choosing the newest row. */
  initialProductionId?: number | null
  /** Embedded continuation used by product-specific creator workflows. */
  ...
  const [visualLevel, setVisualLevel] = useState<'none' | 'key-scenes' | 'illustrated'>(
    props.avgSetup?.settings.visualLevel
      ?? (initialProduct === 'avg' || initialProduct === 'ai-town' || initialProduct === 'text-adventure'
        ? 'key-scenes' : 'none'),
  )
  ...
  const [lockedProductionUnavailable, setLockedProductionUnavailable] = useState(false)
  ...
  const {chatSetup, avgSetup, onProductionSelected} = props
  const refresh = useCallback(async (preferredId?: number | null) => {
    ...
    const lockedProduction = props.productionOnly
      ? props.initialProductionId == null
        ? null
        : nextProductions.find(row => row.id === props.initialProductionId) ?? null
      : null
    const lockedTargetMissing = props.productionOnly === true && lockedProduction == null
    setLockedProductionUnavailable(lockedTargetMissing)
    if (lockedTargetMissing) {
      queuedProductionIdRef.current = null
      productionAbort.current?.abort('locked-product-production-unavailable')
      setError(props.initialProductionId == null
        ? '嵌入式制作入口缺少锁定的 Production ID；已停止，未打开其他制作任务。'
        : `锁定的 Production #${props.initialProductionId} 不存在、已失效或不属于当前产品；已停止，未打开其他制作任务。`)
    }
    const desired = props.productionOnly
      ? lockedProduction?.id ?? null
      : preferredId !== undefined
        ? preferredId
        : selectedProductionIdRef.current != null
            && nextProductions.some(row => row.id === selectedProductionIdRef.current)
          ? selectedProductionIdRef.current
          : props.initialProductionId != null
              && nextProductions.some(row => row.id === props.initialProductionId)
            ? props.initialProductionId
  ...
  }, [allowedProducts, initialSource, onProductSelected, props.initialProductionId, props.productionOnly, scope, chatSetup, avgSetup, onProductionSelected])
  ...
  const compileBrief = () => run(async () => {
    ...
    const next = await compileProductProductionBriefV3({
      ...
      ...(productType === 'avg' && props.avgSetup ? { avg: {...props.avgSetup.settings,playerRole,openingSituation,visualLevel: visualLevel === 'illustrated' ? 'key-scenes' : visualLevel,audioLevel,qualityProfile} } : {}),
  ...
  const inspectTaskEvidence = (taskKey: string) => run(async () => {
    if (!details?.production.id) throw new Error('缺少当前 Production。')
    const rows = await readProductProductionTaskEvidenceV1({
      scope: props.scope,
      productionId: details.production.id,
      taskKey,
    })
    setTaskEvidence({
      taskKey,
      text: rows.length > 0 ? JSON.stringify(rows, null, 2) : '当前任务尚无可展示的冻结证据。',
    })
  }, '读取任务证据')
  ...
  const recoverImportedProofs = () => run(async () => {
    if (!details?.production.id) throw new Error('缺少 Production。')
    const recovered = await recoverImportedProductProductionProofsV1({
      scope: props.scope,
      productionId: details.production.id,
    })
    setProgress(recovered)
    setMessage('导入后的 Run、Artifact、媒资与 Build root 已在新本地作用域完成零调用复验。')
    await refresh(details.production.id)
  }, '复验导入生产证明')
  ...
  const upgradeExecutionPlan = () => run(async () => {
    if (!details) throw new Error('缺少 Production。')
    const created = await upgradeTextAdventureProductionPlanV1({
      scope: props.scope,
      productionId: details.production.id!,
    })
    await refresh(details.production.id)
    setMessage(`旧 Build 保持不可变；执行计划升级已形成 Brief r${created.briefRevision}。授权后将创建子 Build，复用全部可验证成果，仅重跑未签收任务及其质量闭包。`)
  }, '创建执行计划升级 Brief')
  ...
  const regenerateVisualReviewFailures = () => run(async () => {
    ...
    const repairKeys = new Set(visualReviewRepairTargets(visualReview).map(target => target.artifactKey))
    const repairAssets = mediaAssets.filter(asset => repairKeys.has(asset.artifactKey))
    if (!repairAssets.length || repairAssets.length !== repairKeys.size) {
      throw new Error('Visual QA 退回证据与当前冻结图片不完整，不能无证据重生成。')
    }
    const result = await regenerateTextAdventureMediaAssetsV1({
      scope: props.scope, details, assets: repairAssets,
    })
    await refresh(details.production.id)
    setMessage(`Visual QA 的 ${result.artifactKeys.length} 张退回图片已进入受治理返修：Build #${result.parentBuildNumber} 保持不可变，新 Build #${result.buildNumber} 将按逐图审查意见重生成并重跑审计、Visual QA 与装配。`)
  }, '批量修复 Visual QA')
  ...
  const importedProofRecoveryRequired = !!details?.build
  ...
  const recoveryPolicy = useMemo(() => {
    ...
    try {
      return inspectProductProductionBuildRecoveryPolicyV1({
        productType: details.production.productType,
        planJson: details.build.planJson,
        taskKey: blockerTaskKey,
      })
    } catch {
      return null
    }
  }, [blockerTaskKey, details?.build, details?.production.productType])
  ...
  const selectedBrief = useMemo(() => {
    try {
      return details?.executionBrief
        ?? (details?.brief?.briefKind !== 'text-open-world-creator-v1'
          ? JSON.parse(details?.brief?.briefJson ?? 'null') as ProductProductionBriefV3 | null
          : null)
    }
    catch { return null }
  ...
  const visualReviewStatus = visualReviewArtifact?.payload && typeof visualReviewArtifact.payload === 'object'
    ...
    : ''
  const visualRepairTargets = useMemo(
    () => visualReviewRepairTargets(visualReviewArtifact),
    [visualReviewArtifact],
  )
  const visualRepairAssetKeys = useMemo(
    () => new Set(visualRepairTargets.map(target => target.artifactKey)),
    [visualRepairTargets],
  )
  const visualRepairAssets = useMemo(
  ...
  const requestedCommercialImageReady = qualityProfile !== 'commercial-candidate'
    || !['avg', 'ttrpg', 'ai-town', 'text-adventure'].includes(productType)
  ...
  const authorizationReadiness = useMemo(() => {
    ...
    // Creator authorization is completed by its dedicated G5-03/G5-04
    // preflight + frozen-plan flow before this embedded production view opens.
    if (details.brief.briefKind === 'text-open-world-creator-v1') return null
    try {
  ...
  return <div className={`grid min-h-[720px] grid-cols-1 bg-bg-base text-text-primary ${(props.embedded || props.productionOnly) ? 'content-start' : 'lg:grid-cols-[260px_minmax(0,1fr)]'}`} data-testid="product-production-studio">
    {props.productionOnly ? null : props.embedded
      ? <details className="border-b border-border p-5"><summary className="cursor-pointer text-sm">制作记录与新版本</summary><div className="mt-4">{productionDirectory}</div></details>
      : <aside className="border-b border-border bg-bg-surface p-4 lg:border-b-0 lg:border-r">{productionDirectory}</aside>}
    <main className="min-w-0 p-5 md:p-8">
      ...
      /></Suspense>}
      {!details && props.productionOnly
        ? lockedProductionUnavailable
          ? <section role="alert" data-testid="product-production-locked-target-unavailable" className="rounded border border-error/30 bg-error/5 p-5 text-sm text-error">
              锁定的文字开放世界 Production 当前不可用。为避免误执行其他任务，制作工作台已保持关闭。
            </section>
          : <section role="status" className="rounded border border-border bg-bg-elevated p-5 text-sm text-text-muted">
              <Loader2 className="mr-2 inline h-4 w-4 animate-spin" />正在恢复已创建的文字开放世界 Production…
            </section>
        : !details ? <>
        ...
        <section className="grid gap-4 rounded border border-border bg-bg-elevated p-5 md:grid-cols-2">
          ...
          <label className="grid gap-2 text-[10px] text-text-muted">产品形态<select aria-label="产品形态" disabled={props.allowedProducts.length === 1} value={productType} onChange={event => { const next = event.target.value as SupportedProduct; if (!props.allowedProducts.includes(next)) return; const nextScale = next === 'text-adventure' ? 'short-arc' : 'scene'; setProductType(next); setScale(nextScale); setTextAdventureWizard(createDefaultTextAdventureProductionWizardValueV1(nextScale)); props.onProductSelected?.(next); setVisualLevel(next === 'avg' || next === 'ttrpg' || next === 'ai-town' || next === 'text-adventure' ? 'key-scenes' : 'none'); setAudioLevel(next === 'avg' ? 'music-sfx' : 'none'); setConfirmTtrpgDefaultMappings(false); void loadTtrpgProductionWizardV2().then(module => setTtrpgWizard(module.createDefaultTtrpgProductionWizardValueV2())); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary disabled:opacity-70">{props.allowedProducts.map(kind => <option key={kind} value={kind}>{PRODUCT_LABELS[kind]}</option>)}</select><span>{productType === 'ttrpg' ? '跑团使用自己的需求适配器、规则、战役、媒资和运行协议；世界事实只从冻结 WorldRelease 渐进读取。' : productType === 'text-adventure' ? '文字冒险拥有独立的叙事、规则、媒资、Build、Release 与运行状态；只读引用冻结 WorldRelease。' : '每种产品使用自己的需求适配器和生产图，共享中立世界协议但不共享业务表。'}</span></label>
          ...
          <label className="grid gap-2 text-[10px] text-text-muted">游戏规模<select aria-label="游戏规模" value={scale} onChange={event => { const nextScale = event.target.value as ProductProductionScaleV1['scope']; setScale(nextScale); if (productType === 'text-adventure') setTextAdventureWizard(current => ({ ...current, targetPlayMinutes: nextScale === 'chapter' ? 60 : nextScale === 'short-arc' ? 45 : 20 })); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary"><option value="scene">单场景 · 约 20 分钟</option><option value="short-arc">短篇支线</option><option value="chapter">完整章节</option>{productType !== 'text-adventure' && <><option value="multi-chapter">多章节</option><option value="campaign">长线战役</option></>}</select></label>
          ...
          <label className="grid gap-2 text-[10px] text-text-muted">视觉目标<select disabled={!['avg', 'ttrpg', 'ai-town', 'text-adventure'].includes(productType)} value={visualLevel} onChange={event => { setVisualLevel(event.target.value as 'none' | 'key-scenes' | 'illustrated'); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary disabled:opacity-50"><option value="none">纯文字</option><option value="key-scenes">关键场景 + 角色素材</option>{productType === 'text-adventure' && <option value="illustrated">丰富插图</option>}</select><span>{productType === 'avg' || productType === 'ttrpg' || productType === 'ai-town' || productType === 'text-adventure' ? qualityProfile === 'prototype' ? '内容会自主拆分美术需求；原型档使用明确标记的安全占位素材。' : '内容拆分需求后，直接复用全局图片能力并行制作并接入玩家界面。' : '当前产品以专用文字舞台为主。'}</span></label>
          ...
          {productType === 'ttrpg' && ttrpgWizard && <><TtrpgProductionWizard scope={props.scope} value={ttrpgWizard} sourceOptions={sourceOptions} sourceSelection={sourceSelection} onChange={next => { setTtrpgWizard(next); setConfirmTtrpgDefaultMappings(next.confirmAll); setDraft(null) }} /><label className="flex items-start gap-2 rounded border border-border bg-bg-base p-3 text-[10px] leading-5 text-text-muted md:col-span-2"><input type="checkbox" checked={confirmTtrpgDefaultMappings} onChange={event => { setConfirmTtrpgDefaultMappings(event.target.checked); setTtrpgWizard(current => current ? ({ ...current, confirmAll: event.target.checked }) : current); setDraft(null) }} className="mt-1" /><span><strong className="block text-xs text-text-primary">确认第一方规则默认映射</strong>同时确认九步向导末页列出的世界边界、规则许可、AI 身份披露和媒资权利；未确认时 Brief 保持阻塞。</span></label></>}
        ...
        {draft && <section className="mt-5 rounded border border-border bg-bg-elevated p-5">
          <div className="flex items-center justify-between"><h2 className="text-sm font-semibold">Brief v3 审查摘要</h2><code className="text-[9px] text-accent">{draft.intent.productType} / {draft.qualityProfile}</code></div>
          <div className="mt-4 grid gap-3 text-[10px] md:grid-cols-3">
            <article className="rounded bg-bg-base p-3"><strong className="block text-xs">体验</strong><p className="mt-1 text-text-muted">{draft.intent.openingSituation}</p></article>
            <article className="rounded bg-bg-base p-3"><strong className="block text-xs">规模</strong><p className="mt-1 text-text-muted">{draft.aiTown ? `${draft.aiTown.town.residentTarget} 位居民 · ${draft.aiTown.town.majorLocationTarget} 个主要地点` : `${draft.scale.targetPlayMinutes} 分钟 · ${draft.scale.targetEndingCount} 结局`}</p>{draft.textAdventure && <p className="mt-1 text-text-muted">{draft.textAdventure.narrative.targetRegionCount} 大区域 / {draft.textAdventure.narrative.targetAreaCount} 区域 / {draft.textAdventure.narrative.targetLocationCount} 地点 / {draft.textAdventure.narrative.targetSceneCount} 场景</p>}</article>
            <article className="rounded bg-bg-base p-3"><strong className="block text-xs">完成合同</strong><p className="mt-1 text-text-muted">可玩预览 · 媒资覆盖 {Math.round(draft.completionContract.minimumMediaCoverage * 100)}% · {draft.media.imageCount} 张图片</p>{draft.textAdventure && <p className="mt-1 text-text-muted">主线 1 · 支线 {draft.textAdventure.narrative.targetSideQuestCount} · 区域事件 {draft.textAdventure.narrative.targetAmbientEventCount} · {draft.textAdventure.media.mode}</p>}</article>
          </div>
          {draft.aiTown && <div className="mt-3 grid gap-2 text-[10px] md:grid-cols-3" data-testid="ai-town-brief-summary"><span className="rounded bg-bg-base p-2">结局后 {draft.aiTown.continuity.elapsedDays} 天 · {draft.aiTown.clock.actionsPerDay} 次行动/日</span><span className="rounded bg-bg-base p-2">离线 {draft.aiTown.autonomy.offlineEnabled ? `最多 ${draft.aiTown.autonomy.offlineMaximumDays} 日` : '关闭'}</span><span className="rounded bg-bg-base p-2">重大变化人工确认 · 私密心智隔离</span></div>}
          {selectedSuggestion && <p className="mt-4 text-[10px] text-text-muted">起点冲突：{selectedSuggestion.openingConflict}</p>}
          <p className="mt-2 text-[10px] text-text-muted" data-testid="product-production-source-selection-summary">本次通过世界网关冻结 {draft.source.selection.resourceKeys.length} 项语义资源（{frozenSourceFacetSummary(draft, sourceOptions)}）；上层叙事、媒资和运行状态均由产品 Build 自己拥有。</p>
          {draft.unresolvedDecisionKeys.length > 0 && <p className="mt-2 rounded border border-error/30 bg-error/5 p-2 text-[10px] text-error">待确认：{draft.unresolvedDecisionKeys.join('、')}</p>}
        </section>}
        ...
        <section className="mt-5 rounded border border-border bg-bg-elevated p-5">
          ...
          {isTextOpenWorldCreator && details.build && ['preview-ready', 'release-ready', 'released'].includes(details.build.status) && <p className="mt-3 rounded border border-border bg-bg-base p-3 text-[10px] leading-5 text-text-muted" data-testid="text-open-world-creator-release-stage-notice">{details.build.status === 'released' ? '当前 Release 保持不可变。可在下方修改受治理内容并创建子 Build；重新完成媒资、质量与作者确认后发布新版本。' : 'Creator Build 按“产物检查 → 媒资 → 灰盒与质量 → 作者最终确认 → 不可变 Release”顺序收口。'}</p>}
          {importedProofRecoveryRequired && <div className="mt-3 rounded border border-warning/40 bg-warning/5 p-3 text-[10px] leading-5 text-text-muted" data-testid="product-production-import-proof-recovery"><strong className="block text-text-main">导入内容等待本地证明复验</strong><span>备份中的内容与媒资已经恢复；本地 ID 已重绑，复验不会调用模型或媒资供应商。</span></div>}
          ...
          {communityCandidateRepairAvailable && <div className="mt-3 rounded border border-accent/40 bg-accent/5 p-4 text-[10px] leading-5 text-text-primary" data-testid="text-adventure-commercial-media-repair">
            ...
            {details.build?.status === 'cancelled' && <p className="mt-2 rounded border border-success/30 bg-success/5 p-2 text-success">旧 Build 已安全取消；现在可以生成修订候选，任何新生产都会进入不可变子 Build。</p>}
          ...
          {canRetryBlocker && recoveryPolicy?.repairNoteAllowed && <label className="mt-3 block text-xs text-text-muted">本次修复要求（可选）<textarea aria-label="本次修复要求" maxLength={4000} value={repairNote} onChange={event => setRepairNote(event.target.value)} className="mt-2 block min-h-24 w-full rounded border border-border bg-bg-base p-3 text-text-main" placeholder="指出需要修正的内容；仍需遵守已确认的世界、规则和安全边界。" /></label>}
          {canRetryBlocker && recoveryPolicy?.authorDraftAllowed && <details className="mt-3 text-xs text-text-muted"><summary className="cursor-pointer">直接修订任务草稿（高级）</summary><p className="my-2">填写完整 JSON 将跳过本次模型重写，按原任务规则校验；保留原始模型证据，记录为作者修订。只适用于失败的文本任务。</p><textarea aria-label="作者修订的完整任务 JSON" maxLength={120000} value={authorDraftJson} onChange={event => setAuthorDraftJson(event.target.value)} className="min-h-60 w-full rounded border border-border bg-bg-base p-3 font-mono text-text-main" /></details>}
          {providerReservationBlocked && <fieldset className="mt-3 rounded border border-error/30 bg-error/5 p-3 text-xs text-text-muted" data-testid="product-production-unknown-result-disposition"><legend className="px-1 font-medium text-error">先结算供应商请求</legend><p className="mb-2 text-[10px] leading-5">{pausedProviderReservationBlocked ? '暂停时仍有供应商请求在途。恢复前必须把冻结的每个 Run、attempt 和 epoch 一起封账；本次选择应用于全部在途请求。' : observedUncheckpointedResponseBlocked ? '系统已持久化供应商响应证据，但完整用量未能回传。为避免少记账，只能按该 attempt 的冻结预留上限封账。' : '系统不会猜测这次请求是否扣费。请选择与你核对结果一致的处置，选择会与失败 Run、attempt 和 epoch 一起冻结。'}</p>{!observedUncheckpointedResponseBlocked && <label className="mr-4 inline-flex items-center gap-2"><input type="radio" name="unknown-result-disposition" checked={unknownResultDisposition === 'confirmed-not-charged'} onChange={() => setUnknownResultDisposition('confirmed-not-charged')} />已确认未计费</label>}<label className="inline-flex items-center gap-2"><input type="radio" name="unknown-result-disposition" checked={unknownResultDisposition === 'charge-reservation-upper-bound'} onChange={() => setUnknownResultDisposition('charge-reservation-upper-bound')} />{pausedProviderReservationBlocked ? '全部按冻结预留上限封账' : observedUncheckpointedResponseBlocked ? '按冻结预留上限封账并重试' : '无法确认，按预留上限记账'}</label></fieldset>}
          {canRetryBlocker && !recoveryPolicy?.repairNoteAllowed && <p className="mt-3 rounded border border-border bg-bg-base p-3 text-xs text-text-muted" data-testid="product-production-retry-only">当前任务暂不支持修复要求或直接 JSON 修订；可先查看制作记录，再按原冻结输入重试。</p>}
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {details.production.productType === 'text-adventure' && details.build?.status === 'release-ready' && <button disabled={busy || productionRunning} onClick={exportCommunityPrototype} className="flex items-center gap-2 rounded border border-accent/40 bg-accent/10 px-4 py-2 text-xs text-accent disabled:opacity-40"><Download className="h-3.5 w-3.5" />导出社区原型包</button>}
            {details.production.productType === 'text-adventure' && details.production.currentProductReleaseId && <button disabled={busy || productionRunning} onClick={exportCommunityGame} className="flex items-center gap-2 rounded border border-success/40 bg-success/10 px-4 py-2 text-xs text-success disabled:opacity-40"><Download className="h-3.5 w-3.5" />导出正式社区游戏包</button>}
        ...
        {progress && progress.tasks.length > 0 && <section aria-live="polite" className="mt-5 rounded border border-border bg-bg-elevated p-5">
          ...
          <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-3">{progress.tasks.map(task => {
            ...
            return <article key={task.taskKey} className="rounded border border-border bg-bg-base p-3"><span className="flex items-center justify-between gap-2"><strong className="text-[10px]">{presentation.label}</strong><em className={`not-italic text-[9px] ${task.status === 'completed' ? 'text-success' : task.status === 'blocked' ? 'text-error' : 'text-accent'}`}>{statusLabel(task.status)}</em></span><p className="mt-1 text-[9px] text-text-muted">{presentation.owner}</p><code className="mt-1 block text-[8px] text-text-muted">{task.taskKey}</code><p className="mt-2 text-[9px] text-text-muted">{task.lane} · attempt {task.attempt || '—'}{task.blocker ? ` · 阻塞：${task.blocker}` : ''}</p>{task.latestDurableBoundary && <p className="mt-1 text-[9px] text-text-muted">边界：{task.latestDurableBoundary.eventType} #{task.latestDurableBoundary.sequence}</p>}{task.checkpoint && <p className="mt-1 text-[9px] text-text-muted">checkpoint：{task.checkpoint.status} · {task.checkpoint.resumeKind}</p>}{task.runId && <button type="button" disabled={busy} onClick={() => inspectTaskEvidence(task.taskKey)} className="mt-2 rounded border border-border px-2 py-1 text-[9px] text-text-muted disabled:opacity-40">查看冻结证据</button>}</article>
          })}</div>
          {taskEvidence && <div className="mt-4 rounded border border-border bg-bg-base p-3" data-testid="product-production-task-evidence"><div className="flex items-center justify-between gap-2"><strong className="text-[10px]">{taskEvidence.taskKey} · 冻结证据</strong><button type="button" onClick={() => setTaskEvidence(null)} className="text-[9px] text-text-muted">关闭</button></div><pre className="mt-2 max-h-80 overflow-auto whitespace-pre-wrap text-[9px] leading-5 text-text-muted">{taskEvidence.text}</pre></div>}
        </section>}
        ...
        {details?.production.productType === 'text-adventure' && mediaAssets.length > 0 && <section className="mt-5 rounded border border-border bg-bg-elevated p-5" data-testid="text-adventure-media-authoring">
          ...
          {visualRepairTargets.length > 0 && visualReviewStatus !== 'passed' && <div className="mt-4 rounded border border-error/30 bg-error/5 p-4 text-[10px]" data-testid="text-adventure-visual-repair">
            <div className="flex flex-wrap items-start justify-between gap-3"><div><strong className="text-xs text-error">Visual QA 退回 {visualRepairTargets.length} 张图片</strong><p className="mt-1 max-w-3xl leading-5 text-text-muted">系统会冻结下列逐图证据，只重生成这些图片；原始需求、角色锚点与其余已通过素材继续沿用。新 Build 会自动重跑需求绑定审计、独立多模态审图与装配。</p></div><button type="button" disabled={busy || productionRunning || visualRepairAssets.length !== visualRepairTargets.length} onClick={regenerateVisualReviewFailures} className="flex items-center gap-2 rounded border border-error/40 bg-error/10 px-4 py-2 text-xs text-error disabled:opacity-40"><RefreshCw className="h-3.5 w-3.5" />按审查意见批量返修</button></div>
            <ul className="mt-3 grid gap-2 md:grid-cols-2">{visualRepairTargets.map(target => <li key={target.artifactKey} className="rounded border border-border bg-bg-base p-3"><span className="flex items-center justify-between gap-2"><strong>{target.artifactKey}</strong><em className="not-italic text-error">{target.verdict}</em></span>{target.issues.length > 0 ? <ul className="mt-2 grid gap-1 text-text-muted">{target.issues.map((issue, index) => <li key={`${target.artifactKey}:${index}`}><strong className={issue.severity === 'blocking' ? 'text-error' : 'text-accent'}>{issue.severity === 'blocking' ? '阻塞' : '警告'}：</strong>{issue.detail}<span className="block pl-8">返修：{issue.recommendation}</span></li>)}</ul> : <p className="mt-2 text-text-muted">需按审图 verdict 重新生成并再次观察。</p>}</li>)}</ul>
          </div>}
        ...
        {isTextOpenWorldCreator && details.build && <TextOpenWorldCreatorArtifactBrowser
          ...
          onRepairBuildCreated={() => refresh(details.production.id)}
        />}
        {isTextOpenWorldCreator && details.build
        ...
        {isTextOpenWorldCreator && details.build
          && details.production.status === 'preview-ready'
          && details.build.status === 'release-ready'
          && <TextOpenWorldCreatorReleaseStudio
            scope={props.scope}
            productionId={selectedProductionId!}
            buildId={details.build.id!}
            refreshToken={`${details.build.id}:${details.build.stateRevision}:${details.production.stateRevision}`}
            ...
            onPublished={async receipt => {
              await refresh(details.production.id)
              setMessage(`ProductRelease v${receipt.releaseVersion} 已原子发布；玩家端读取同一 RuntimePackage。`)
              props.onPublished?.('text-open-world')
            }}
          />}
