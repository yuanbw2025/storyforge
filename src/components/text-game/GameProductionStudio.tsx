    ...
    <main className="min-w-0 p-5 md:p-8">
      ...
      {!details ? <>
        ...
        <section className="grid gap-4 rounded border border-border bg-bg-elevated p-5 md:grid-cols-2">
          ...
          <label className="grid gap-2 text-[10px] text-text-muted">产品形态<select value={productType} onChange={event => { const next = event.target.value as SupportedProduct; setProductType(next); setVisualLevel(next === 'avg' || next === 'ttrpg' ? 'key-scenes' : 'none'); setAudioLevel(next === 'avg' ? 'music-sfx' : 'none'); setConfirmTtrpgDefaultMappings(false); setTtrpgWizard(createDefaultTtrpgProductionWizardValueV2()); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary"><option value="storygame">分支叙事</option><option value="character-interaction">角色互动</option><option value="text-adventure">文字冒险</option><option value="avg">轻量 AVG（含媒资）</option><option value="narrative-simulation">叙事模拟</option><option value="text-open-world">文字开放世界</option><option value="ttrpg">跑团 CampaignPack</option></select><span>{productType === 'ttrpg' ? '跑团使用自己的需求适配器、规则、战役、媒资和运行协议；世界事实只从冻结 WorldRelease 渐进读取。' : '每种产品使用自己的需求适配器和生产图，共享中立世界协议但不共享业务表。'}</span></label>
