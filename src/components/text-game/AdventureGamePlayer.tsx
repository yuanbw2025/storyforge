...
export default function AdventureGamePlayer(props: {
  ...
  const skillPointDefinition = adventureV2?.resources.find(item => item.key === adventureV2.progression.skillPointResourceKey)
  ...
  >
    ...
    {panel && <div className="adventure-panel-backdrop" role="presentation"><section className="adventure-panel" aria-label={{ character: '角色状态', world: '区域地图', inventory: '背包', equipment: '装备', skills: '技能', quests: '任务', relationships: '人物关系', journal: '冒险记录', ending: '结局因果回顾', accessibility: '阅读与可访问性', saves: '存档与时间线' }[panel]}><header><div><small>ADVENTURER'S JOURNAL</small><h2>{{ character: '角色状态', world: '区域与地点', inventory: '背包与物品', equipment: '装备栏', skills: '角色能力', quests: '任务日志', relationships: '人物关系', journal: '冒险记录', ending: '结局因果回顾', accessibility: '阅读与可访问性', saves: '存档与时间线' }[panel]}</h2></div><button aria-label="关闭面板" onClick={() => setPanel(null)}><X /></button></header><div className="adventure-panel-content">
      ...
      {panel === 'inventory' && <div className="adventure-inventory-grid">{playerItems.map(item => { const definition = manifest.adventure.items.find(value => value.key === item.itemKey); const itemActions = actions.filter(candidate => candidate.action.targetKey === item.itemKey && ['use', 'give'].includes(candidate.action.kind)); return <article key={`${item.itemKey}:${item.state}`}><i>{itemIcon(definition?.tags ?? [], item.itemKey)}</i><div><small>{item.state === 'equipped' ? '已装备' : definition?.consumable ? '消耗品' : '携带中'}</small><strong>{friendlyName(definition?.title, item.itemKey)}</strong><p>{friendlyDescription(definition?.description, item.itemKey, 'item')}</p>{itemActions.length > 0 && <span className="adventure-item-actions">{itemActions.map(action => <button key={action.action.key} disabled={!action.available} title={!action.available ? presentationText(action.reason) : undefined} onClick={() => void executeAction(action.action.key)}>{presentationText(action.action.label)}</button>)}</span>}{itemActions.some(action => !action.available) && <small className="adventure-item-action-reason">{itemActions.filter(action => !action.available).map(action => presentationText(action.reason)).filter(Boolean).join('；')}</small>}</div><b>×{item.quantity}</b></article> })}{!playerItems.length && <div className="adventure-empty">背包还是空的，探索场景会找到可携带的物品。</div>}</div>}
      {panel === 'equipment' && adventureV2 && <div className="adventure-system-grid">{adventureV2.equipmentSlots.map(slot => { const entry = playerItems.find(item => item.state === 'equipped' && adventureV2.items.find(definition => definition.key === item.itemKey)?.equipmentSlotKey === slot.key); const definition = entry ? adventureV2.items.find(item => item.key === entry.itemKey) : null; const availableEquipment = playerItems.filter(item => item.state !== 'equipped' && adventureV2.items.find(candidate => candidate.key === item.itemKey)?.equipmentSlotKey === slot.key); const selectedAction = entry ? actions.find(candidate => candidate.action.key.startsWith('action.unequip.') && candidate.action.targetKey === entry.itemKey) : availableEquipment.map(item => actions.find(candidate => candidate.action.key.startsWith('action.equip.') && candidate.action.targetKey === item.itemKey)).find(Boolean); return <article key={slot.key}><small>{slot.title}</small><strong>{definition?.title ?? '未装备'}</strong><p>{definition?.description ?? `可放置带有 ${slot.acceptsTags.join('、') || '兼容'} 标签的装备。`}</p>{definition?.modifiers.map(modifier => <span key={modifier.abilityKey}>{adventureV2.abilities.find(item => item.key === modifier.abilityKey)?.title ?? modifier.abilityKey} {modifier.delta > 0 ? '+' : ''}{modifier.delta}</span>)}{selectedAction && <><button className="adventure-panel-action" disabled={!selectedAction.available} title={!selectedAction.available ? presentationText(selectedAction.reason) : undefined} onClick={() => void executeAction(selectedAction.action.key)}>{presentationText(selectedAction.action.label)}</button>{!selectedAction.available && <small className="adventure-item-action-reason">{presentationText(selectedAction.reason)}</small>}</>}</article> })}</div>}
      {panel === 'skills' && <div className="adventure-skills-panel">
        {adventureV2 && <header><div><small>可分配技能点</small><strong>{skillPointBalance}</strong></div><p>技能点由正式任务奖励获得；每次提升消耗 1 点并写入当前时间线。</p></header>}
        <div className="adventure-skills-grid">{visibleAbilities.filter(item => !adventureV2 || abilityRole(item.definition.key) === 'skill').map(({ definition, base, effective }) => {
          const atMaximum = base >= definition.maximum
          return <article key={definition.key}><i><WandSparkles /></i><div><small>能力等级 {effective}{effective !== base ? ` · 基础 ${base}` : ''}</small><strong>{friendlyName(definition.title, definition.key)}</strong><p>{friendlyDescription(definition.description, definition.key, 'ability')}</p><span><b style={{ width: `${gauge(effective, definition.minimum, definition.maximum)}%` }} /></span>{adventureV2 && <button
            className="adventure-skill-upgrade"
            aria-label={`提升${friendlyName(definition.title, definition.key)}`}
            disabled={store.busy || narrativeReading || skillPointBalance < 1 || atMaximum}
            title={atMaximum ? '该技能已经达到上限' : skillPointBalance < 1 ? `需要 1 ${skillPointDefinition?.title ?? '技能点'}` : `消耗 1 ${skillPointDefinition?.title ?? '技能点'}`}
            onClick={() => void run(() => store.allocateSkillPoint(definition.key))}
          >{atMaximum ? '已达上限' : `提升 · 1 ${skillPointDefinition?.title ?? '技能点'}`}</button>}</div></article>
        })}</div>
      </div>}
      {panel === 'quests' && <div className="adventure-quest-list">{questGroups.map(group => <section className="adventure-quest-group" key={group.key}>
        <h3>{group.title}<span>{group.quests.length}</span></h3>
        {group.quests.map(quest => {
          const definition = manifest.adventure.quests.find(item => item.key === quest.questKey)
          const activeObjective = quest.objectives.find(item => !item.completed && !item.optional)
            ?? quest.objectives.find(item => !item.completed)
          const activeDefinition = definition?.objectives.find(item => item.key === activeObjective?.objectiveKey)
          const activeStage = adventureV2?.quests.find(item => item.key === quest.questKey)?.stages
            .find(stage => stage.objectiveKeys.includes(activeObjective?.objectiveKey ?? ''))
          const objectiveRoutes = (activeDefinition?.alternativeActionKeys ?? []).flatMap(actionKey => {
            const action = manifest.adventure.actions.find(item => item.key === actionKey)
            if (!action || !isAdventureActionPlayerVisible(manifest, action)) return []
            const actionLocation = manifest.adventure.locations.find(item => item.key === action.locationKey)
            const atCurrentLocation = action.locationKey === adventure.currentLocationKey
            const requirementsMet = action.requirements.every(requirement => adventureRequirementSatisfied(
              requirement,
              adventure,
              adventureNarrativeActionContext(store.runtimeState.narrative),
              manifest.adventure,
            ))
            const alreadyCompleted = !action.repeatable && adventure.completedActionKeys.includes(action.key)
            const available = atCurrentLocation && requirementsMet && !alreadyCompleted
            let ruleLabel = '无需判定'
            if (action.rule.kind === 'resource-payment') {
              const rule = action.rule
              const resource = manifest.adventure.resources.find(item => item.key === rule.resourceKey)
              ruleLabel = `消耗 ${resource?.title ?? rule.resourceKey} ${rule.amount}`
            } else if (action.rule.kind === 'threshold' || action.rule.kind === 'random') {
              const rule = action.rule
              const ability = manifest.adventure.abilities.find(item => item.key === rule.abilityKey)
              ruleLabel = `${ability?.title ?? rule.abilityKey} ${rule.kind === 'random' ? rule.expression + '，难度 ' : '门槛 '}${rule.difficulty}`
            }
            const failureForward = action.failureEffects.some(effect => effect.op === 'complete-objective'
              && effect.questKey === quest.questKey && effect.objectiveKey === activeObjective?.objectiveKey)
            const failureStopsQuest = action.failureEffects.some(effect => effect.op === 'fail-quest'
              && effect.questKey === quest.questKey)
            const statusLabel = available
              ? '现在可执行'
              : !atCurrentLocation && actionLocation
                ? `前往${actionLocation.title}后尝试`
                : alreadyCompleted
                  ? '该路线已经结算'
                  : action.unavailableText
            return [{ action, actionLocation, available, ruleLabel, statusLabel, failureForward, failureStopsQuest }]
          })
          const firstRouteLocation = objectiveRoutes.find(route => route.actionLocation)?.actionLocation
          return <article className={'status-' + quest.status} key={quest.questKey}>
            <header><span>{QUEST_STATUS[quest.status]}</span><strong>{definition?.title ?? quest.questKey}</strong></header>
            <p>{definition?.description}</p>
            {activeObjective && <div className="adventure-quest-current">
              <small>{activeStage ? '当前阶段 · ' + activeStage.title : '当前目标'}</small>
              <b>{activeDefinition?.title ?? activeObjective.objectiveKey}</b>
              {firstRouteLocation && <span>地点：{firstRouteLocation.title}</span>}
            </div>}
            {!!objectiveRoutes.length && <div className="adventure-quest-routes">
              <small>可行路线 · {objectiveRoutes.length}</small>
              {objectiveRoutes.map((route, index) => <button
                key={route.action.key}
                disabled={!route.available || store.busy || narrativeReading}
                title={route.statusLabel}
                onClick={() => void executeAction(route.action.key)}
              >
                <span><i>路线 {index + 1}</i><b>{presentationText(route.action.label)}</b><small>{presentationText(route.action.description)}</small></span>
                <em>{route.ruleLabel}</em>
                <strong className={route.available ? 'available' : ''}>{route.statusLabel}</strong>
                {route.failureForward && <mark>失败有代价，但仍会推进目标</mark>}
                {route.failureStopsQuest && <mark className="danger">失败可能终止任务</mark>}
              </button>)}
            </div>}
            <ul>{quest.objectives.map(item => <li className={item.completed ? 'done' : ''} key={item.objectiveKey}>{item.completed ? <Check /> : <CircleDot />}{definition?.objectives.find(value => value.key === item.objectiveKey)?.title ?? item.objectiveKey}</li>)}</ul>
          </article>
        })}
      </section>)}</div>}
