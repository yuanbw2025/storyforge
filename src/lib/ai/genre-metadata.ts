 ...
 * 用于 AI 生成时注入额外约束。
 */
import { buildNearModernTechConstraintBlock } from './near-modern-tech-reference'
...
export const GENRE_METADATA: GenreMetadata[] = [
  ...
  // ── 冷战氛围（近现代科技异世界，非地球史复刻） ──
  {
    id: 'coldwar',
    label: '冷战氛围',
    emoji: '📡',
    description: '近现代科技异世界：阵营对峙、信息延迟、体制摩擦与科技/能力竞赛；不复刻地球冷战史',
    antiPatterns: [
      '禁止互联网/智能手机/GPS/实时全球检索等数字时代便利；技术上限约等于前数字时代（近现代）',
      '不要复刻地球冷战专名与年表（国家名、机构缩写、具体历史事件），除非作者明确要求写地球史',
      '不要把阵营写成脸谱化善恶对立；各方内部应有派系、程序与利益冲突',
      '不要让情报/魔法充当随身全知网络：远距通信有延迟、有代价、可被截获',
      '不要把终极威慑手段当背景板——它应改变交涉、部署与保密红线，而不是反复渲染末日情绪',
      '不要用情绪形容词硬撑氛围（压抑、绝望、沉郁循环）；用技术限制与制度程序推动情节',
      '允许魔法/异能，但通信、侦察、计算须遵守与近现代科技同等的延迟与代价约束',
    ],
    pacingStrategy: '以任务周期推进：目标下达→资源/情报受限→局部突破→反制与程序代价→新的稳定对峙。高潮后回到可执行的下一步（补洞、洗牌、换线），不靠情绪悬空。',
    typicalStructure: [
      { title: '分界', description: '两大（或多）阵营的边界、缓冲带与公开规则确立；主角进入体制或夹缝位置' },
      { title: '竞赛', description: '科技/能力/情报优势的争夺；代理人冲突、走私、学术掩护与黑市并行' },
      { title: '越线', description: '一次越权行动或情报失手打破表面平衡；程序、上级与敌方同时加压' },
      { title: '改约', description: '规则重谈、阵营重组或红线重划；旧问题未消，新约束写进日常' },
    ],
  },
]
...
export function buildGenreConstraintContext(genreIds: string | string[]): string {
  ...
  return Array.from(metas.values()).map(meta => {
    ...
    if (meta.id === 'coldwar') {
      parts.push(buildNearModernTechConstraintBlock())
    }
