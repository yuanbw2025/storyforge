...
interface ImpactRemediationOutputV1 {
  planHash: string
  graphHash: string
...
export async function executeImpactRemediationV1(input: {
  ...
  const existing = await readExistingCompletedRun(input.scope, input.plan)
  if (existing?.projection.terminalReceiptHash) {
    ...
    if (receiptEvent?.payload.receiptHash) {
      return {
        snapshot: existing,
        ...
        output: {
          ...
          retrieval: { rebuilt: false, count: 0 },
          summaries: { chapterNodes: 0, volumeNodes: 0, bookNodes: 0, staleNodes: 0 },
        },
        reused: true,
      }
    }
  ...
  const snapshotCreated = await createAgentRunV1({
    scope: input.scope,
    worldGroupId: input.worldGroupId,
  ...
  let snapshot = await append(input.scope, snapshotCreated, 'step.scheduled', { stepId: IMPACT_REMEDIATION_STEP_ID_V1 })
  ...
  try {
    ...
    const output: ImpactRemediationOutputV1 = {
      planHash: input.plan.planHash,
    ...
    const receipt = await createVerificationReceiptV1({
      ...
      verifierSetVersion: IMPACT_REMEDIATION_VERIFIER_SET_V1,
      criteria: [
        { id: 'impact-remediation.deterministic', status: 'passed', evidenceRefs: [`output:${outputHash}`, `plan:${input.plan.planHash}`] },
