...
export function buildShortNovelPromptV1(input: {
  ...
  if (input.kind === 'brief') {
    objective = `生成 ShortNovelBriefV1。字段严格且仅为：${SHORT_NOVEL_CONTRACT_KEYS.BRIEF_KEYS.join(', ')}。目标字数必须沿用 Work 的 5,000～25,000；章节数 3～8；coreChange 表达主人公经历后不可逆的变化；storyPromise 可在结尾检验。`
  } else if (input.kind === 'story-design') {
    objective = `生成 ShortNovelStoryDesignV1。字段严格且仅为：${SHORT_NOVEL_CONTRACT_KEYS.DESIGN_KEYS.join(', ')}。用一个主角、一条主冲突和有限角色完成闭环；escalation 给出 2～5 次压力升级；climaxChoice 必须迫使主角用行动回答 thematicQuestion；endingImage 与开场形成变化对照。`
    ...
    objective = `为 ${input.chapterKey} 生成 ShortNovelChapterDraftV1，字段严格且仅为：${SHORT_NOVEL_CONTRACT_KEYS.DRAFT_KEYS.join(', ')}。content 使用纯文本自然段，不要 Markdown 标题；严格执行该章结构卡、承接前章离场状态并把下一章所需状态交代清楚；不提前兑现后续高潮。`
  } else if (input.kind === 'continuity-review') {
    objective = `生成 ShortNovelReviewV1。顶层字段严格且仅为：${SHORT_NOVEL_CONTRACT_KEYS.REVIEW_KEYS.join(', ')}；每个 issue 字段严格且仅为：${SHORT_NOVEL_CONTRACT_KEYS.ISSUE_KEYS.join(', ')}。severity 仅 critical/major/minor；category 仅 causality/character/continuity/pacing/point-of-view/promise-payoff/prose；status 固定 open。evidence 必须引用短小的实际文本或结构事实；没有问题时 issues=[]，不得为凑数发明问题。`
  } else {
    objective = `只修复下列问题，并为 ${input.chapterKey} 返回 ShortNovelChapterDraftV1，字段严格且仅为：${SHORT_NOVEL_CONTRACT_KEYS.DRAFT_KEYS.join(', ')}。保持未涉及的情节事实、视角、语气和长度，不改其他章节。问题：${JSON.stringify(input.issue)}`
  }
