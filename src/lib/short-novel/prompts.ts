...
export function buildShortNovelPromptV1(input: {
  ...
  if (input.kind === 'brief') {
    ...
    objective = `生成 ShortNovelReviewV1。顶层必须精确采用这个 JSON 形状：{"version":1,"summary":"非空总结","strengths":["非空优点"],"issues":[]}，不得增加 score、verdict、recommendations 等键。每个 issue 必须精确采用这个 JSON 形状：{"stableKey":"issue-1","severity":"major","category":"continuity","chapterKeys":["chapter-1"],"evidence":"原文写道：“实际短引文”","problem":"非空问题","suggestion":"非空建议","status":"open"}，字段严格且仅为：${SHORT_NOVEL_CONTRACT_KEYS.ISSUE_KEYS.join(', ')}；绝对不要增加 title、location、line、priority、confidence 或 reasoning。version 必须是数字 1；severity 仅逐字使用 critical/major/minor；category 仅逐字使用 causality/character/continuity/pacing/point-of-view/promise-payoff/prose；status 固定为英文 open。evidence 必须包含至少一个用引号括起、逐字存在于 chapterKeys 对应正文或结构卡中的短引文；不得改写、拼接或推测不存在的句子。没有问题时 issues=[]，不得为凑数发明问题。`
  } else {
