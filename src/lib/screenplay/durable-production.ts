...
function prompt(stage: ScreenplayProfessionalStageV1, context: string, targetKeys: string[], authorInstruction: string, targetSpec: unknown): ChatMessage[] {
  ...
  const instructions: Record<ScreenplayProfessionalStageV1, string> = {
    ...
    'beat-sheet': '输出 Beat 数组。每项严格含 stableKey, sectionKey, sectionTitle, scope, episodeNumber, order, objective, conflict, turn, outcome, causalFactKeys, decisionKeys, sourceUnitKeys, estimatedSeconds。电影集号固定 1；分集按每集引擎和集尾推进；总时长服从目标画像。',
    'scene-card': '把每个 Beat 展开为一个或多个 Scene Card。每项严格含 stableKey, beatKey, episodeNumber, sceneNumber, order, purpose, conflict, entryState, exitState, visibleAction, informationReveal, sourceUnitKeys, estimatedSeconds。进入/退出状态必须可连续核对，可视动作不能是小说内心说明。',
    'scene-draft': `只写目标 Scene Card 的一场 AST 对象，字段严格仅为 ${SCENE_KEYS.join(', ')}。stableKey/集号/场号/时长/来源服从 Card；blocks 仅 action/character/parenthetical/dialogue/transition/shot/note。action 只写可见可听可执行内容；character→可选 parenthetical→dialogue；不复制来源长对白。`,
    'grounding-review': '只做来源忠实度与连续性审查。输出 ReviewIssue 数组，每项严格含 stableKey, category, severity, sceneKey, blockId, evidence, problem, suggestion, sourceUnitKeys；category 固定 grounding。每项必须定位 scene/block 并给出来源证据；合理且已确认的 add 不得误报。没有问题输出 []。',
    'dramaturgy-review': '只做戏剧审查：场景目的、冲突、动作化、节奏、人物声音与对白功能。输出 ReviewIssue 数组，字段同审查契约，category 固定 dramaturgy；evidence 引用目标场景文本，不臆造来源问题。没有问题输出 []。',
    'targeted-rewrite': '只修复作者选中的开放问题。输出对象严格含 sceneKey, expectedSceneRevision, issueKeys, summary, blocks。保持 sceneKey、场序、人物事实、来源、未授权台词与结构不变；不得顺手重写别场。',
  }
