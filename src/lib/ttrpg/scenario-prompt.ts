...
质量要求：
0. 所有 key、seatKey、nodeKey、actionKey、属性 key 与各类 *Keys 引用必须匹配 ^[A-Za-z0-9][A-Za-z0-9._:-]{0,199}$，只用 ASCII 标识（例如 npc.xu_wei），中文姓名只放 name。同一个 scene + action 组合最多对应一条线索；不得让一项检定产生两条不同线索。
