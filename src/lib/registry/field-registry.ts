/** FIELD_REGISTRY · 当前 AI/结构化采纳允许写入字段的单一事实源。 */
import type { FieldSpec } from './types'

const trimString = (val: unknown): unknown =>
  typeof val === 'string' ? val.trim() : val

function text(target: string, field: string, labels?: string[]): FieldSpec {
  return { target, field, type: 'string', labels, sanitize: trimString }
}

function longtext(target: string, field: string, labels?: string[]): FieldSpec {
  return { target, field, type: 'longtext', labels, sanitize: trimString }
}

function num(target: string, field: string, labels?: string[]): FieldSpec {
  return { target, field, type: 'number', labels }
}

function bool(target: string, field: string, labels?: string[]): FieldSpec {
  return { target, field, type: 'boolean', labels }
}

function json(target: string, field: string, labels?: string[]): FieldSpec {
  return { target, field, type: 'json', labels }
}

/** IndexedDB 原生对象字段（区别于以 JSON string 存储的 json 字段）。 */
function object(target: string, field: string, labels?: string[]): FieldSpec {
  return { target, field, type: 'object', labels }
}

function arr(target: string, field: string, labels?: string[]): FieldSpec {
  return { target, field, type: 'array', labels }
}
...
function enumeration(
  ...
  return { target, field, type: 'enum', enums, labels, sanitize: trimString }
}
...
export const WORLDVIEW_GENERATABLE_FIELD_SPECS = [
  {
    target: 'worldviews', field: 'worldOrigin', type: 'longtext', labels: ['世界来源', '世界起源'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'powerHierarchy', type: 'longtext', labels: ['力量体系'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'divineDesign', type: 'object', labels: ['神明设定'],
  ...
  {
    target: 'worldviews', field: 'worldStructure', type: 'longtext', labels: ['世界结构'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'worldDimensions', type: 'longtext', labels: ['世界尺寸'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'continentLayout', type: 'longtext', labels: ['地貌分布', '大陆分布'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'regionDimensions', type: 'longtext', labels: ['区域面积'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'mountainsRivers', type: 'longtext', labels: ['山川河流'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'climateByRegion', type: 'longtext', labels: ['气候'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'naturalResourceOverview', type: 'longtext', labels: ['自然资源概述', '自然资源全貌'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'naturalResources', type: 'object', labels: ['自然资源'],
  ...
  {
    target: 'worldviews', field: 'races', type: 'longtext', labels: ['种族'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'factionLayout', type: 'longtext', labels: ['势力分布'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'politicsOverview', type: 'longtext', labels: ['政治概述', '政治制度概述'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'economyOverview', type: 'longtext', labels: ['经济概述', '经济制度概述'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'cultureOverview', type: 'longtext', labels: ['文化概述', '文化制度概述'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'internalConflicts', type: 'longtext', labels: ['内部矛盾'], sanitize: trimString,
  ...
  {
    target: 'worldviews', field: 'itemDesign', type: 'longtext', labels: ['道具设计'], sanitize: trimString,
...
export const STORY_CORE_GENERATABLE_FIELD_SPECS = [
  {
    target: 'storyCores', field: 'logline', type: 'longtext', labels: ['一句话故事'], sanitize: trimString,
  ...
  {
    target: 'storyCores', field: 'concept', type: 'longtext', labels: ['故事概念'], sanitize: trimString,
  ...
  {
    target: 'storyCores', field: 'theme', type: 'longtext', labels: ['主题'], sanitize: trimString,
  ...
  {
    target: 'storyCores', field: 'centralConflict', type: 'longtext', labels: ['核心冲突'], sanitize: trimString,
  ...
  {
    target: 'storyCores', field: 'plotPattern', type: 'longtext', labels: ['情节模式'], sanitize: trimString,
  ...
  {
    target: 'storyCores', field: 'mainPlot', type: 'longtext', labels: ['主线', '故事主线'], sanitize: trimString,
  ...
  {
    target: 'storyCores', field: 'subPlots', type: 'longtext', labels: ['复线'], sanitize: trimString,
...
export const FIELD_REGISTRY: FieldSpec[] = [
  ...
  // SCREEN-PRO-1: professional screenplay planning/review content. Stable
  // identity, owner, manifest, author status and revisions remain system-owned.
  text('screenplayBeats', 'sectionKey'),
  longtext('screenplayBeats', 'sectionTitle'),
  enumeration('screenplayBeats', 'scope', ['act', 'sequence', 'episode']),
  num('screenplayBeats', 'episodeNumber'),
  num('screenplayBeats', 'order'),
  longtext('screenplayBeats', 'objective'),
  longtext('screenplayBeats', 'conflict'),
  longtext('screenplayBeats', 'turn'),
  longtext('screenplayBeats', 'outcome'),
  arr('screenplayBeats', 'causalFactKeys'),
  arr('screenplayBeats', 'decisionKeys'),
  arr('screenplayBeats', 'sourceUnitKeys'),
  num('screenplayBeats', 'estimatedSeconds'),
  text('screenplaySceneCards', 'beatKey'),
  num('screenplaySceneCards', 'episodeNumber'),
  num('screenplaySceneCards', 'sceneNumber'),
  num('screenplaySceneCards', 'order'),
  longtext('screenplaySceneCards', 'purpose'),
  longtext('screenplaySceneCards', 'conflict'),
  longtext('screenplaySceneCards', 'entryState'),
  longtext('screenplaySceneCards', 'exitState'),
  longtext('screenplaySceneCards', 'visibleAction'),
  longtext('screenplaySceneCards', 'informationReveal'),
  arr('screenplaySceneCards', 'sourceUnitKeys'),
  num('screenplaySceneCards', 'estimatedSeconds'),
  enumeration('screenplayReviewIssues', 'category', ['grounding', 'continuity', 'dramaturgy', 'format']),
  enumeration('screenplayReviewIssues', 'severity', ['critical', 'major', 'minor']),
  text('screenplayReviewIssues', 'sceneKey'),
  text('screenplayReviewIssues', 'blockId'),
  longtext('screenplayReviewIssues', 'evidence'),
  longtext('screenplayReviewIssues', 'problem'),
  longtext('screenplayReviewIssues', 'suggestion'),
  arr('screenplayReviewIssues', 'sourceUnitKeys'),
  ...
  text('comicPages', 'pagePlanKey'),
  object('comicPanels', 'frame'),
  text('comicPanels', 'nextPanelKey'),
  object('comicPanels', 'shot'),
  text('comicPanels', 'narrativeFunction'),
  longtext('comicPanels', 'moment'),
  ...
  arr('comicPanels', 'subjectStates'),
  arr('comicPanels', 'protectedAreas'),
  ...
  text('comicScriptBeats', 'sectionKey'),
  text('comicScriptBeats', 'narrativeFunction'),
  longtext('comicScriptBeats', 'visualAction'),
  longtext('comicScriptBeats', 'dialogueIntent'),
  text('comicScriptBeats', 'emotion'),
  arr('comicScriptBeats', 'causalFactKeys'),
  arr('comicScriptBeats', 'decisionKeys'),
  arr('comicScriptBeats', 'sourceUnitKeys'),
  longtext('comicPagePlans', 'goal'),
  arr('comicPagePlans', 'beatKeys'),
  longtext('comicPagePlans', 'endReveal'),
  text('comicPagePlans', 'pageTurn'),
  text('comicReviewIssues', 'category'),
  text('comicReviewIssues', 'severity'),
  text('comicReviewIssues', 'pageKey'),
  text('comicReviewIssues', 'panelKey'),
  text('comicReviewIssues', 'subjectKey'),
  text('comicReviewIssues', 'assetKey'),
  longtext('comicReviewIssues', 'evidence'),
  longtext('comicReviewIssues', 'problem'),
  longtext('comicReviewIssues', 'suggestion'),
  arr('comicReviewIssues', 'sourceUnitKeys'),

  // MOTION-DRAMA-1: model output is always a reviewed candidate. Stable
  // identity, owner, manifest, author status, revisions, provider receipts,
  // media selection and release state remain system-owned.
  object('motionDramaSeriesBibles', 'bible'),
  text('motionDramaEpisodes', 'title'),
  longtext('motionDramaEpisodes', 'logline'),
  longtext('motionDramaEpisodes', 'synopsis'),
  longtext('motionDramaEpisodes', 'openingHook'),
  arr('motionDramaEpisodes', 'beats'),
  longtext('motionDramaEpisodes', 'endHook'),
  arr('motionDramaEpisodes', 'continuityIn'),
  arr('motionDramaEpisodes', 'continuityOut'),
  arr('motionDramaEpisodes', 'sourceUnitKeys'),
  longtext('motionDramaScriptScenes', 'heading'),
  text('motionDramaScriptScenes', 'location'),
  text('motionDramaScriptScenes', 'timeOfDay'),
  longtext('motionDramaScriptScenes', 'dramaticPurpose'),
  longtext('motionDramaScriptScenes', 'entryState'),
  longtext('motionDramaScriptScenes', 'exitState'),
  longtext('motionDramaScriptScenes', 'visibleAction'),
  arr('motionDramaScriptScenes', 'dialogue'),
  longtext('motionDramaScriptScenes', 'narration'),
  arr('motionDramaScriptScenes', 'soundCues'),
  longtext('motionDramaScriptScenes', 'emotionalTurn'),
  num('motionDramaScriptScenes', 'estimatedSeconds'),
  arr('motionDramaScriptScenes', 'characterKeys'),
  arr('motionDramaScriptScenes', 'sourceUnitKeys'),
  enumeration('motionDramaAssetSubjects', 'kind', ['character', 'costume', 'location', 'prop', 'style', 'voice', 'sound']),
  text('motionDramaAssetSubjects', 'label'),
  longtext('motionDramaAssetSubjects', 'identity'),
  longtext('motionDramaAssetSubjects', 'appearance'),
  arr('motionDramaAssetSubjects', 'palette'),
  arr('motionDramaAssetSubjects', 'materials'),
  arr('motionDramaAssetSubjects', 'continuityLocks'),
  arr('motionDramaAssetSubjects', 'prohibitedChanges'),
  longtext('motionDramaAssetSubjects', 'basePrompt'),
  longtext('motionDramaAssetSubjects', 'negativePrompt'),
  longtext('motionDramaAssetSubjects', 'referenceBrief'),
  arr('motionDramaAssetSubjects', 'sourceUnitKeys'),
  longtext('motionDramaShots', 'narrativeFunction'),
  num('motionDramaShots', 'targetSeconds'),
  enumeration('motionDramaShots', 'shotSize', ['extreme-wide', 'wide', 'full', 'medium', 'close-up', 'extreme-close-up', 'insert']),
  enumeration('motionDramaShots', 'cameraAngle', ['eye-level', 'high', 'low', 'overhead', 'dutch', 'pov']),
  enumeration('motionDramaShots', 'cameraMovement', ['static', 'pan', 'tilt', 'track', 'dolly', 'orbit', 'zoom', 'handheld']),
  longtext('motionDramaShots', 'composition'),
  longtext('motionDramaShots', 'visibleAction'),
  longtext('motionDramaShots', 'performance'),
  longtext('motionDramaShots', 'lighting'),
  longtext('motionDramaShots', 'transitionIn'),
  longtext('motionDramaShots', 'transitionOut'),
  longtext('motionDramaShots', 'dialogue'),
  longtext('motionDramaShots', 'narration'),
  arr('motionDramaShots', 'soundPlan'),
  arr('motionDramaShots', 'subjectKeys'),
  arr('motionDramaShots', 'sourceUnitKeys'),
  longtext('motionDramaShots', 'imagePrompt'),
  longtext('motionDramaShots', 'negativeImagePrompt'),
  longtext('motionDramaShots', 'firstFramePrompt'),
  longtext('motionDramaShots', 'keyFramePrompt'),
  longtext('motionDramaShots', 'lastFramePrompt'),
  longtext('motionDramaShots', 'videoPrompt'),
  longtext('motionDramaShots', 'negativeVideoPrompt'),
  enumeration('motionDramaReviewIssues', 'category', ['story', 'continuity', 'visual', 'motion', 'prompt', 'sound', 'rights', 'provider-capability']),
  enumeration('motionDramaReviewIssues', 'severity', ['critical', 'major', 'minor']),
  text('motionDramaReviewIssues', 'sceneKey'),
  text('motionDramaReviewIssues', 'shotKey'),
  text('motionDramaReviewIssues', 'subjectKey'),
  longtext('motionDramaReviewIssues', 'evidence'),
  longtext('motionDramaReviewIssues', 'problem'),
  longtext('motionDramaReviewIssues', 'suggestion'),
  ...
  longtext('characters', 'shortDescription', ['简介', '一句话简介']),
  longtext('characters', 'appearance', ['外貌']),
  longtext('characters', 'personality', ['性格']),
  longtext('characters', 'background', ['背景']),
  ...
  num('characters', 'homeWorldGroupId', ['所属世界ID']),
  ...
  // creativeRules
  longtext('creativeRules', 'writingStyle', ['文风']),
  enumeration(
    'creativeRules',
    'narrativePOV',
  ...
  longtext('creativeRules', 'atmosphere', ['氛围', '基调']),
  ...
  enumeration('outlineNodes', 'type', ['volume', 'arc', 'storyBlock', 'chapter'], ['节点类型']),
  ...
  enumeration('chapters', 'status', ['outline', 'draft', 'revised', 'polished', 'final'], ['状态']),
  num('chapters', 'order'),
  ...
  enumeration('foreshadows', 'type', ['chekhov', 'prophecy', 'symbol', 'character', 'dialogue', 'environment', 'timeline', 'red-herring', 'parallel', 'callback']),
  enumeration('foreshadows', 'status', ['planned', 'planted', 'echoed', 'resolved']),
  ...
  enumeration('storyArcs', 'type', ['main', 'sub']),
  ...
  enumeration('codexCategories', 'domain', ['natural', 'humanity', 'origin']),
  ...
  enumeration('itemLedger', 'action', ['gain', 'consume']),
