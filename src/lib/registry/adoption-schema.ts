...
export const ADOPTION_EXTENSIONS: readonly AdoptionExtensionSpec[] = Object.freeze([
  ...
  },
  {
    ...
    target: 'adaptationProjects',
    entrypoints: ['src/lib/adaptation/source-manifest.ts', 'src/lib/adaptation/completion.ts', 'src/lib/adaptation/analysis.ts', 'src/lib/screenplay/production.ts', 'src/lib/screenplay/release.ts'],
  ...
  },
  {
    ...
    policyRegistry: 'SHORT_NOVEL_RELEASE_V1 + SCREENPLAY_RELEASE_V1 + PROJECT_TABLES + completion gates + immutable hash',
    reason: '独立短篇与剧本发布都必须从已审校 Canon 原子冻结，自包含且 append-only；所有格式导出只能读取已验证 manifest。',
    reviewAfter: '2027-09-06',
  },
  {
