...
import { effectiveWorkspacePurpose, generateWorkspaceScopeCode, generateWorldCode } from './identity'
import { generateWorkCode, generateWorkspaceUid, isWorkCode, isWorkspaceUid } from '../memory/identity'
import { obsoleteProjectIdentityFieldsV1 } from '../migrations/project-root-authority'
...
/**
 * Native workspaces do not pass through the imported-data migration, but scope
...
function stableFingerprintPayload(project: Project, rows: Map<string, CapturedRow[]>): string {
  ...
  const projectFields = Object.fromEntries(PROJECT_ROOT_FIELDS.map(field => [
    field,
...
function requiresOwnerStamp(spec: TableSpec): boolean {
  ...
  return spec.domainOwner?.defaultOwner === 'world' || spec.domainOwner?.defaultOwner === 'work'
}
...
function assertNoUnknownOwners(state: WorkspaceState): void {
  for (const spec of PROJECT_TABLES) {
    if (!requiresOwnerStamp(spec)) continue
    for (const row of rowsFor(state, spec.name)) {
...
function buildBeforeImage(project: Project): Record<string, OwnershipBeforeImageValue> {
  ...
  return Object.fromEntries(PROJECT_ROOT_FIELDS.map(field => [field, {
...
async function allocateWorldRootCode(projectId: number, shareable: boolean): Promise<string> {
  for (let attempt = 0; attempt < 8; attempt += 1) {
    const code = shareable ? generateWorldCode() : generateWorkspaceScopeCode()
    const collision = await db.worlds.where('code').equals(code).first()
    if (!collision || collision.projectId === projectId) return code
  }
  fail('OWNERSHIP_WORLD_CODE_COLLISION', '无法分配唯一的世界编号')
}

async function allocateWorkRootCode(projectId: number): Promise<string> {
  for (let attempt = 0; attempt < 8; attempt += 1) {
    const code = generateWorkCode()
    const collision = await db.works.where('[projectId+code]').equals([projectId, code]).first()
    if (!collision) return code
  }
  fail('OWNERSHIP_WORK_CODE_COLLISION', '无法分配唯一的作品编号')
}

async function stampMigratedOwners(plan: OwnershipMigrationPlan, worldId: number, workId: number): Promise<void> {
  for (const spec of PROJECT_TABLES) {
    if (!requiresOwnerStamp(spec)) continue
    ...
    const changes = domain.defaultOwner === 'world'
      ? { worldId }
...
function assertStampedOwners(state: WorkspaceState, worldId: number, workId: number): void {
  for (const spec of PROJECT_TABLES) {
    ...
    if (!domain) continue
    for (const row of rowsFor(state, spec.name)) {
      const actual = domain.defaultOwner === 'world' ? row.worldId : row.workId
      const expected = domain.defaultOwner === 'world' ? worldId : workId
...
async function runOwnershipMigration(plan: OwnershipMigrationPlan, receiptId: number): Promise<WorkspaceOwnershipResolution> {
  ...
  try {
    return await db.transaction('rw', transactionTablesFor('deleteProject'), async () => {
      ...
      const identityKind = workspacePurpose === 'world-engine' ? 'world-draft' : 'workspace-scope'
      const now = Date.now()
      ...
      const existingWork = plan.existingWorkId == null ? undefined : await db.works.get(plan.existingWorkId)
      if (plan.existingWorkId != null && !existingWork) {
        fail('OWNERSHIP_UNKNOWN_ROOTS', '迁移准备阶段登记的 Work 已不存在')
      }
      const worldCode = existingWorld?.code?.trim()
        || await allocateWorldRootCode(projectId, identityKind === 'world-draft')
      const worldVersion = existingWorld?.currentVersion ?? 0
      const worldId = plan.existingWorldId ?? await db.worlds.add({
        ...
        communityOrigin: existingWorld?.communityOrigin,
        createdAt: normalized.createdAt ?? now,
        updatedAt: normalized.updatedAt ?? now,
      }) as number
      if (existingWorld && (existingWorld.identityKind !== identityKind || existingWorld.code !== worldCode)) {
        await db.worlds.update(existingWorld.id!, { identityKind, code: worldCode, updatedAt: now })
      }
      const workCode = existingWork && isWorkCode(existingWork.code)
        ? existingWork.code
        : await allocateWorkRootCode(projectId)
      ...
      if (existingWork && existingWork.code !== workCode) {
        await db.works.update(existingWork.id!, { code: workCode, updatedAt: now })
      }

      await stampMigratedOwners(plan, worldId, workId)
      await db.projects.update(projectId, {
        ...
        workspacePurpose,
        ...obsoleteProjectIdentityFieldsV1(),
      } as Partial<Project>)
...
/** One-way first-entry upgrader. Repeated calls for a ready workspace perform no writes. */
...
/** Resolve a project-only route through the mandatory ownership contract. */
