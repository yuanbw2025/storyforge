...
export function parseTextOpenWorldModulesV1(value: TextOpenWorldRuntimePackageV1 | string | unknown): TextOpenWorldParsedModulesV1 {
  ...
  conditions.forEach(item => canonicalGameProductionJsonV2(item.expression))
  effects.forEach((item, index) => { key(item.operation, `actions.effects[${index}].operation`); canonicalGameProductionJsonV2(item.payload) })
  ...
  const progressionRules = row(progression.rules, 'progression.rules'); exact(progressionRules, ['maximumLevel', 'automaticAttributeGrowth', 'attributes', 'formulas'], 'progression.rules'); const maximumLevel = int(progressionRules.maximumLevel, 'progression.rules.maximumLevel', 1, 20); if (progressionRules.automaticAttributeGrowth !== true) fail('首版必须自动属性成长')
  ...
  if (canonicalGameProductionJsonV2({ morality: relationships.morality, factionAffinity: relationships.factionAffinity, attitude: relationships.attitude })
    !== canonicalGameProductionJsonV2({ morality: packageValue.calibration.relationships.morality, factionAffinity: packageValue.calibration.relationships.factionAffinity, attitude: packageValue.calibration.relationships.attitude })) {
