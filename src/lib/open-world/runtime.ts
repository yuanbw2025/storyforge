...
type WorldDescriptor = {
  type: Extract<import('../types').ProductRuntimeEventType, `world.${string}`> | Extract<import('../types').ProductRuntimeEventType, `adventure.${string}`>
...
function fail(message: string): never { throw new Error(`[text-open-world] ${message}`) }
...
function parseContent(value: unknown): OpenWorldContentV1 {
  ...
  exact(source, ['version', 'initialRegionKey', 'tickLimit', 'evolutionCadenceTicks', 'maxPropagationEdgesPerTick', 'regions', 'travelEdges', 'discoveryChannels', 'fixedTaskCards', 'taskTemplates', 'decks', 'actorSchedules', 'regionalIssueRules', 'mainline', 'director'], '开放世界内容')
  ...
  return {
    version: 1, initialRegionKey: key(source.initialRegionKey, 'initialRegionKey'), tickLimit: integer(source.tickLimit, 'tickLimit', 1, 100_000), evolutionCadenceTicks: integer(source.evolutionCadenceTicks, 'evolutionCadenceTicks', 1, 10_000), maxPropagationEdgesPerTick: integer(source.maxPropagationEdgesPerTick, 'maxPropagationEdgesPerTick', 1, 1_000), regions, travelEdges, discoveryChannels, fixedTaskCards, taskTemplates, decks, actorSchedules, regionalIssueRules,
...
export function validateOpenWorldContent(input: {
  ...
  const regionKeys = new Set(content.regions.map(item => item.key)); const edgeKeys = new Set(content.travelEdges.map(item => item.key)); const channelKeys = new Set(content.discoveryChannels.map(item => item.key)); const profileKeys = new Set(input.interactionProfiles.map(item => item.participantKey)); const organizationKeys = new Set(input.openWorldEvolution.actors.filter(item => item.kind === 'organization').map(item => item.key)); const issueKeys = new Set(input.openWorldEvolution.issues.map(item => item.key)); const resourceKeys = new Set(input.openWorldEvolution.resources.map(item => item.key)); const metricKeys = new Set(input.openWorldEvolution.metrics.map(item => item.key)); const questKeys = new Set(input.adventure.quests.map(item => item.key)); const locationKeys = new Set(input.adventure.locations.map(item => item.key)); const cardKeys = new Set(content.fixedTaskCards.map(item => item.key)); const templateKeys = new Set(content.taskTemplates.map(item => item.key))
...
export function createInitialOpenWorldState(contentValue: OpenWorldContentV1 | string, contentHash: string): OpenWorldRuntimeState {
...
export function parseOpenWorldState(value: unknown): OpenWorldRuntimeState | null {
  ...
  return structuredClone({ ...source, schema: 'storyforge.open-world', version: 1, contentHash: source.contentHash, mainlineQuestKeys: keys(source.mainlineQuestKeys, 'mainlineQuestKeys', 100), tick: integer(source.tick, 'tick'), tickLimit: integer(source.tickLimit, 'tickLimit', 1), currentRegionKey: key(source.currentRegionKey, 'currentRegionKey'), travel, regionKnowledge, attentionLevels, regionalProjections: projections, actorLocations, questInstances, drawHistory: array(source.drawHistory, 'drawHistory', 256) as OpenWorldRuntimeState['drawHistory'], remoteActionHistory: array(source.remoteActionHistory, 'remoteActionHistory', 256) as OpenWorldRuntimeState['remoteActionHistory'], propagationHistory: array(source.propagationHistory, 'propagationHistory', 256) as OpenWorldRuntimeState['propagationHistory'], recentFingerprints: array(source.recentFingerprints, 'recentFingerprints', 512) as OpenWorldRuntimeState['recentFingerprints'], sourceCooldowns: numberMap(source.sourceCooldowns, 'sourceCooldowns'), blankStreak: integer(source.blankStreak, 'blankStreak'), highIntensityStreak: integer(source.highIntensityStreak, 'highIntensityStreak'), lastTickEventSequences: array(source.lastTickEventSequences, 'lastTickEventSequences', 1_000).map((item, index) => integer(item, `lastTickEventSequences[${index}]`)), ended: bool(source.ended, 'ended') } as OpenWorldRuntimeState)
}

function stateRequired(value: OpenWorldRuntimeState | null): OpenWorldRuntimeState { return parseOpenWorldState(value) ?? fail('当前实例没有开放世界状态') }
function eventPayload(event: ProductRuntimeEvent) { try { return row(JSON.parse(event.payloadJson), `事件 ${event.type}`) } catch (error) { if (error instanceof SyntaxError) fail('事件载荷不是 JSON'); throw error } }
function projection(state: OpenWorldRuntimeState, regionKey: string) { return state.regionalProjections.find(item => item.regionKey === regionKey) ?? fail(`区域投影不存在:${regionKey}`) }
...
function applyChanges(state: OpenWorldRuntimeState, changes: ValueChange[], sequence: number) { for (const change of changes) { const target = projection(state, change.regionKey); const map = change.target === 'resource' ? target.resources : change.target === 'metric' ? target.metrics : change.target === 'issue' ? target.issuePressures : target.organizationInfluence; if (map[change.key] !== change.before) fail(`区域变化前值不一致:${change.regionKey}:${change.key}`); map[change.key] = change.after; target.lastUpdatedSequence = sequence } }

export function applyOpenWorldEvent(value: OpenWorldRuntimeState | null, event: ProductRuntimeEvent): OpenWorldRuntimeState | null {
...
export function openWorldNarrativeProjection(stateValue: OpenWorldRuntimeState) {
...
export function openWorldMainlineProjection(stateValue: OpenWorldRuntimeState, mainlineQuestKeys: readonly string[]) {
...
function conditionSatisfied(condition: OpenWorldCondition, state: OpenWorldRuntimeState): boolean {
...
function event(sequence: number, type: WorldDescriptor['type'], payload: Record<string, unknown>, actorKey: string | null = null, targetKey: string | null = null): ProductRuntimeEvent { return { projectId: 0, sessionId: 0, sequence, type, actorKey, targetKey, payloadJson: JSON.stringify(payload), createdAt: 0 } }
function applyDescriptor(state: OpenWorldRuntimeState, descriptor: WorldDescriptor, sequence: number) { return stateRequired(applyOpenWorldEvent(state, event(sequence, descriptor.type, descriptor.payload, descriptor.actorKey ?? null, descriptor.targetKey ?? null))) }

function valueBounds(openWorldEvolution: OpenWorldEvolutionContentV1, target: ValueChange['target'], entryKey: string) { if (target === 'organization') return { minimum: -100, maximum: 100 }; const values = target === 'resource' ? openWorldEvolution.resources : target === 'metric' ? openWorldEvolution.metrics : openWorldEvolution.issues.map(issue => ({ key: issue.key, minimum: issue.minimumPressure, maximum: issue.maximumPressure })); return values.find(item => item.key === entryKey) ?? fail(`变化目标不存在:${target}:${entryKey}`) }
function changesForEffects(state: OpenWorldRuntimeState, effects: readonly OpenWorldEffect[], openWorldEvolution: OpenWorldEvolutionContentV1): ValueChange[] {
  const working = stateRequired(state); const changes: ValueChange[] = []
  for (const effect of effects) { const target = projection(working, effect.regionKey); const kind: ValueChange['target'] = effect.op === 'change-organization-influence' ? 'organization' : effect.target; const entryKey = effect.op === 'change-organization-influence' ? effect.organizationKey : effect.key; const map = kind === 'resource' ? target.resources : kind === 'metric' ? target.metrics : kind === 'issue' ? target.issuePressures : target.organizationInfluence; const before = map[entryKey] ?? fail(`区域变化目标不存在:${effect.regionKey}:${entryKey}`); const after = before + effect.delta; const bounds = valueBounds(openWorldEvolution, kind, entryKey); if (after < bounds.minimum || after > bounds.maximum) fail(`区域变化越界:${effect.regionKey}:${entryKey}`); const change = { target: kind, regionKey: effect.regionKey, key: entryKey, before, after, delta: effect.delta }; changes.push(change); map[entryKey] = after }
  return changes
}
...
function terminalOrMissingAdventureQuest(adventure: AdventureRuntimeState, questKey: string) { const status = adventure.quests.find(item => item.questKey === questKey)?.status; return status == null || status === 'active' || status === 'completed' || status === 'failed' }

export function planOpenWorldDraw(input: { content: OpenWorldContentV1 | string; openWorldEvolution: OpenWorldEvolutionContentV1; state: OpenWorldRuntimeState; adventure: AdventureRuntimeState; trigger: OpenWorldDiscoveryTrigger; seed: string; startingSequence: number }): { descriptors: WorldDescriptor[]; projected: OpenWorldRuntimeState; evidence: OpenWorldDirectorCandidateEvidence[] } {
...
export function planOpenWorldTick(input: { content: OpenWorldContentV1 | string; openWorldEvolution: OpenWorldEvolutionContentV1; state: OpenWorldRuntimeState; seed: string; startingSequence: number }): { descriptors: WorldDescriptor[]; projected: OpenWorldRuntimeState } { const content = parseOpenWorldContent(input.content); let state = stateRequired(input.state); if (state.ended) fail('世界长局已经结束'); const nextTick = state.tick + 1; const descriptors: WorldDescriptor[] = []; const push = (descriptor: WorldDescriptor) => { descriptors.push(descriptor); state = applyDescriptor(state, descriptor, input.startingSequence + descriptors.length) }
