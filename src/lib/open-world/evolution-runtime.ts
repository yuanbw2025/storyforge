/**
 * Text-open-world private state-evolution kernel.
 *
 * The OpenWorldEvolution names below describe a capability/data protocol;
 * they are not a product identity, release kind, session kind or UI entry.
 */
import type {
  OpenWorldEvolutionActionDefinition,
  OpenWorldEvolutionActorActionDefinition,
  OpenWorldEvolutionAtomicEffect,
  OpenWorldEvolutionCondition,
  OpenWorldEvolutionContentV1,
  OpenWorldEvolutionEffect,
  OpenWorldEvolutionEndingDefinition,
  OpenWorldEvolutionIssueDefinition,
  OpenWorldEvolutionModifierDefinition,
  OpenWorldEvolutionPresentationCandidateV1,
  OpenWorldEvolutionReport,
  OpenWorldEvolutionScheduledEffect,
  OpenWorldEvolutionValidationReport,
  ProductRuntimeEvent,
  ProductRuntimeEventType,
  OpenWorldEvolutionState,
} from '../types'

type Row = Record<string, unknown>
type ProductRuntimeEventDescriptor = {
  type: Extract<ProductRuntimeEventType, `open-world-evolution.${string}`>
...
function fail(message: string): never { throw new Error(`[text-open-world/evolution] ${message}`) }
...
function parseCondition(value: unknown, label: string): OpenWorldEvolutionCondition {
...
function parseAtomicEffect(value: unknown, label: string): OpenWorldEvolutionAtomicEffect {
  const source = row(value, label)
...
function parseEffect(value: unknown, label: string): OpenWorldEvolutionEffect {
  const source = row(value, label)
...
function parseAction(value: unknown, label: string): OpenWorldEvolutionActionDefinition {
  const source = row(value, label)
...
function parseActorAction(value: unknown, label: string): OpenWorldEvolutionActorActionDefinition {
  const source = row(value, label)
...
function parseContent(value: unknown): OpenWorldEvolutionContentV1 {
  ...
  const modifiers = array(source.modifiers, 'modifiers', 100).map((item, index): OpenWorldEvolutionModifierDefinition => {
  ...
  const issues = array(source.issues, 'issues', 100).map((item, index): OpenWorldEvolutionIssueDefinition => {
  ...
  const endings = array(source.endings, 'endings', 32).map((item, index): OpenWorldEvolutionEndingDefinition => {
...
export function parseOpenWorldEvolutionContent(value: string | unknown): OpenWorldEvolutionContentV1 {
  let parsed = value
...
function effectReferences(effect: OpenWorldEvolutionEffect | OpenWorldEvolutionAtomicEffect) {
...
export function validateOpenWorldEvolutionContent(input: {
  content: OpenWorldEvolutionContentV1 | string
  narrativeNodeKeys?: readonly string[]
}): OpenWorldEvolutionValidationReport {
  const content = parseOpenWorldEvolutionContent(input.content)
  ...
  const checkCondition = (condition: OpenWorldEvolutionCondition) => {
  ...
  const checkEffect = (effect: OpenWorldEvolutionEffect | OpenWorldEvolutionAtomicEffect) => {
  ...
  const signature = (action: OpenWorldEvolutionActionDefinition) => JSON.stringify({ requirements: action.requirements, immediateEffects: action.immediateEffects, delayedEffects: action.delayedEffects, cooldownTurns: action.cooldownTurns })
...
function issueStage(issue: OpenWorldEvolutionIssueDefinition, pressure: number) {
...
const CURRENT_INTERNAL_SIMULATION_SCHEMA = 'storyforge.text-open-world.evolution' as const

export function createInitialOpenWorldEvolutionState(contentInput: OpenWorldEvolutionContentV1 | string, contentHash: string): OpenWorldEvolutionState {
  const content = parseOpenWorldEvolutionContent(contentInput)
  if (!/^[a-f0-9]{64}$/.test(contentHash)) fail('contentHash 无效')
  return {
    schema: CURRENT_INTERNAL_SIMULATION_SCHEMA, version: 1, contentHash, turn: 1, turnLimit: content.turnLimit,
...
export function parseOpenWorldEvolutionState(value: unknown): OpenWorldEvolutionState | null {
  if (value == null) return null
  const source = row(value, '演化状态')
  const schema = String(source.schema)
  if (schema !== CURRENT_INTERNAL_SIMULATION_SCHEMA
    || source.version !== 1 || typeof source.contentHash !== 'string'
    || !/^[a-f0-9]{64}$/.test(source.contentHash)) fail('演化状态版本或内容哈希无效')
  const state = {
    ...source,
    schema: CURRENT_INTERNAL_SIMULATION_SCHEMA,
  } as unknown as OpenWorldEvolutionState
  ...
  if (!Array.isArray(state.activeModifiers) || !Array.isArray(state.issues) || !Array.isArray(state.reports)
    ...
    || !Array.isArray(state.lastTurnEventSequences) || !state.cooldowns || typeof state.cooldowns !== 'object') fail('演化状态集合无效')
  return structuredClone(state)
}

function eventPayload(event: ProductRuntimeEvent): Row {
...
function requireState(current: OpenWorldEvolutionState | null): OpenWorldEvolutionState {
  const state = parseOpenWorldEvolutionState(current)
  if (!state) fail('事件缺少文字开放世界内部状态演化状态')
  return state
}

export function applyOpenWorldEvolutionProductRuntimeEvent(current: OpenWorldEvolutionState | null, event: ProductRuntimeEvent): OpenWorldEvolutionState {
  ...
  switch (event.type) {
    case 'open-world-evolution.turn.started': {
    ...
    case 'open-world-evolution.decision.committed': {
    ...
    case 'open-world-evolution.decision.rejected': remember(); break
    case 'open-world-evolution.resource.changed':
    case 'open-world-evolution.metric.changed': {
      if (state.phase !== 'resolving') fail('数值只能在结算阶段变化')
      const target = event.type === 'open-world-evolution.resource.changed' ? state.resources : state.metrics
    ...
    case 'open-world-evolution.modifier.applied': {
      const modifier = row(payload.modifier, 'modifier') as unknown as OpenWorldEvolutionState['activeModifiers'][number]
    ...
    case 'open-world-evolution.modifier.ticked': {
    ...
    case 'open-world-evolution.modifier.expired':
    case 'open-world-evolution.modifier.removed': {
    ...
    case 'open-world-evolution.effect.scheduled': {
      const schedule = row(payload.schedule, 'schedule') as unknown as OpenWorldEvolutionScheduledEffect
    ...
    case 'open-world-evolution.effect.settled': {
    ...
    case 'open-world-evolution.actor.action-resolved': {
    ...
    case 'open-world-evolution.issue.created': {
    ...
    case 'open-world-evolution.issue.stage-changed': {
    ...
    case 'open-world-evolution.issue.resolved': {
    ...
    case 'open-world-evolution.report.created': {
      const report = row(payload.report, 'report') as unknown as OpenWorldEvolutionReport
    ...
    case 'open-world-evolution.ending.qualified': {
      if (state.qualifiedEndingKey) fail('结局已确定')
      state.qualifiedEndingKey = key(payload.endingKey, 'endingKey'); remember(); break
    }
    case 'open-world-evolution.narrative.synced': remember(); break
    case 'open-world-evolution.turn.ended': {
    ...
    default: fail(`未知文字开放世界内部状态演化事件:${event.type}`)
  }
  return state
}
...
export function evaluateOpenWorldEvolutionCondition(condition: OpenWorldEvolutionCondition, state: OpenWorldEvolutionState): boolean {
...
export function availableOpenWorldEvolutionActions(contentInput: OpenWorldEvolutionContentV1 | string, stateInput: OpenWorldEvolutionState) {
  const content = parseOpenWorldEvolutionContent(contentInput); const state = requireState(stateInput)
  return content.actions.map(action => {
    const failed = action.requirements.find(condition => !evaluateOpenWorldEvolutionCondition(condition, state))
...
function chooseActorAction(actions: OpenWorldEvolutionActorActionDefinition[], seed: string, turn: number, actorKey: string) {
...
function valueDefinition(content: OpenWorldEvolutionContentV1, target: 'resource' | 'metric', targetKey: string) {
  return (target === 'resource' ? content.resources : content.metrics).find(item => item.key === targetKey)
}

export interface PlanOpenWorldEvolutionTurnInput {
  content: OpenWorldEvolutionContentV1 | string
  state: OpenWorldEvolutionState
  decisionKeys: string[]
  seed: string
  startingSequence: number
}

export function planOpenWorldEvolutionTurn(input: PlanOpenWorldEvolutionTurnInput): {
  descriptors: ProductRuntimeEventDescriptor[]
  projected: OpenWorldEvolutionState
  ending: OpenWorldEvolutionEndingDefinition | null
} {
  const content = parseOpenWorldEvolutionContent(input.content); let projected = requireState(input.state)
  ...
  const availability = new Map(availableOpenWorldEvolutionActions(content, projected).map(item => [item.action.key, item]))
  ...
  const descriptors: ProductRuntimeEventDescriptor[] = []
  const emit = (type: ProductRuntimeEventDescriptor['type'], payload: Record<string, unknown>, actorKey: string | null = null, targetKey: string | null = null, commandEnvelope = false) => {
    ...
    projected = applyOpenWorldEvolutionProductRuntimeEvent(projected, { projectId: 0, sessionId: 0, sequence, type, actorKey, targetKey, payloadJson: JSON.stringify(payload), createdAt: 0 })
    return sequence
  }
  const turn = projected.turn
  emit('open-world-evolution.turn.started', { turn, decisionKeys: input.decisionKeys }, 'player', null, true)
  const applyAtomic = (effect: OpenWorldEvolutionAtomicEffect, sourceKey: string) => {
    if (effect.op === 'change-value') {
      ...
      emit(effect.target === 'resource' ? 'open-world-evolution.resource.changed' : 'open-world-evolution.metric.changed', { turn, key: effect.key, before, delta: effect.delta, after, sourceKey }, null, effect.key)
      ...
      if (after !== issue.pressure || nextStage?.key !== issue.stageKey) emit('open-world-evolution.issue.stage-changed', { turn, issueKey: issue.issueKey, beforePressure: issue.pressure, afterPressure: after, fromStageKey: issue.stageKey, toStageKey: nextStage?.key ?? issue.stageKey, sourceKey }, null, issue.issueKey)
      if (after === definition.minimumPressure && !projected.issues.find(item => item.issueKey === issue.issueKey)?.resolved) emit('open-world-evolution.issue.resolved', { turn, issueKey: issue.issueKey, sourceKey }, null, issue.issueKey)
    } else {
      const sourceEventSequences = [...projected.lastTurnEventSequences]
      const report: OpenWorldEvolutionReport = { reportId: `${effect.reportKey}:${turn}:${input.startingSequence + descriptors.length + 1}`, reportKey: effect.reportKey, turn, observerKey: effect.observerKey, visibility: effect.visibility, text: effect.text, confidence: effect.confidence, sourceEventSequences, expiresAtTurn: effect.expiresAfterTurns == null ? null : turn + effect.expiresAfterTurns }
      emit('open-world-evolution.report.created', { turn, report }, effect.observerKey === 'player' ? 'system' : effect.observerKey, effect.reportKey)
    }
  }
  for (const schedule of projected.schedules.filter(item => item.status === 'pending' && item.dueTurn <= turn)) {
    ...
    emit('open-world-evolution.effect.settled', { turn, scheduleId: schedule.scheduleId }, null, schedule.scheduleId)
  }
  for (const active of [...projected.activeModifiers]) {
    ...
    if (active.remainingTurns <= 1) emit('open-world-evolution.modifier.expired', { turn, instanceKey: active.instanceKey }, null, active.instanceKey)
    else emit('open-world-evolution.modifier.ticked', { turn, instanceKey: active.instanceKey, remainingTurns: active.remainingTurns - 1 }, null, active.instanceKey)
  }
  const applyEffect = (effect: OpenWorldEvolutionEffect, sourceActionKey: string) => {
    ...
    emit('open-world-evolution.modifier.applied', { turn, modifier: { instanceKey, modifierKey: definition.key, sourceActionKey, appliedTurn: turn, remainingTurns: definition.durationTurns, stacks: definition.stackMode === 'stack' ? 1 : definition.stackMode === 'refresh' ? (previous?.stacks ?? 1) : 1 } }, null, instanceKey)
  }
  for (const actionKey of input.decisionKeys) {
    const action = content.actions.find(item => item.key === actionKey)!
    emit('open-world-evolution.decision.committed', { turn, actionKey, availableTurn: Math.min(content.turnLimit + 1, turn + action.cooldownTurns + 1) }, 'player', actionKey)
    ...
    action.delayedEffects.forEach((delayed, index) => {
      const schedule: OpenWorldEvolutionScheduledEffect = { scheduleId: `${actionKey}:${turn}:${index}`, sourceActionKey: actionKey, dueTurn: turn + delayed.afterTurns, effects: delayed.effects, status: 'pending', createdSequence: input.startingSequence + descriptors.length + 1, settledSequence: null }
      emit('open-world-evolution.effect.scheduled', { turn, schedule }, 'player', schedule.scheduleId)
    })
  }
  for (const actor of content.actors) {
    const available = actor.strategyActions.filter(action => action.requirements.every(condition => evaluateOpenWorldEvolutionCondition(condition, projected)))
    const action = chooseActorAction(available, input.seed, turn, actor.key); if (!action) continue
    const actionSequence = emit('open-world-evolution.actor.action-resolved', { turn, actorKey: actor.key, actionKey: action.key, strategyEvidence: `fnv1a:${input.seed}:${turn}:${actor.key}` }, actor.key, action.key)
  ...
  emit('open-world-evolution.report.created', { turn, report: { reportId: `turn-summary:${turn}`, reportKey: 'turn-summary', turn, observerKey: 'player', visibility: 'player', text: `第 ${turn} 回合已结算：${input.decisionKeys.length ? input.decisionKeys.join('、') : '未安排主动决策'}。`, confidence: 1, sourceEventSequences: summaryEvidence, expiresAtTurn: null } satisfies OpenWorldEvolutionReport }, 'system', 'turn-summary')
  const ending = [...content.endings].sort((left, right) => right.priority - left.priority || left.key.localeCompare(right.key))
    .find(candidate => candidate.conditions.every(condition => evaluateOpenWorldEvolutionCondition(condition, projected))) ?? null
  if (ending) emit('open-world-evolution.ending.qualified', { turn, endingKey: ending.key, narrativeNodeKey: ending.narrativeNodeKey, evidenceSequences: [...projected.lastTurnEventSequences] }, null, ending.key)
  emit('open-world-evolution.narrative.synced', { turn, projection: openWorldEvolutionProjection(projected) })
  const nextSequence = input.startingSequence + descriptors.length + 1
  emit('open-world-evolution.turn.ended', { turn, nextTurn: ending ? turn : turn + 1, eventSequences: [...projected.lastTurnEventSequences, nextSequence] })
  return { descriptors, projected, ending }
}

export function openWorldEvolutionProjection(stateInput: OpenWorldEvolutionState) {
...
export function visibleOpenWorldEvolutionReports(stateInput: OpenWorldEvolutionState, observerKey = 'player') {
  const state = requireState(stateInput)
 ...
 * provenance remains on ProductRuntimeSession.parentThroughSequence. */
export function rebaseOpenWorldEvolutionStateForBranch(
  stateInput: OpenWorldEvolutionState,
): OpenWorldEvolutionState {
  const state = requireState(stateInput)
  return {
    ...state,
...
export function validateOpenWorldEvolutionPresentationCandidate(input: {
  candidate: OpenWorldEvolutionPresentationCandidateV1
  state: OpenWorldEvolutionState
  ...
  if (!candidate.evidenceEventSequences.length || candidate.evidenceEventSequences.some(sequence => !evidence.get(sequence)?.type.startsWith('open-world-evolution.'))) fail('AI 表现候选引用了不存在或越界事件')
...
export function runOpenWorldEvolutionBatch(input: {
  content: OpenWorldEvolutionContentV1 | string
  contentHash: string
  seed: string
  turns: number
  decide: (state: OpenWorldEvolutionState, availableActionKeys: string[]) => string[]
}) {
  const content = parseOpenWorldEvolutionContent(input.content); let state = createInitialOpenWorldEvolutionState(content, input.contentHash)
  let sequence = 0; const events: ProductRuntimeEvent[] = []
  for (let index = 0; index < input.turns && state.phase !== 'ended'; index += 1) {
    const available = availableOpenWorldEvolutionActions(content, state).filter(item => item.available).map(item => item.action.key)
    const plan = planOpenWorldEvolutionTurn({ content, state, decisionKeys: input.decide(structuredClone(state), available), seed: input.seed, startingSequence: sequence })
