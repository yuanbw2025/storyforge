...
export interface GameProductionConsultationSourceV2 {
  schema: 'storyforge.game-production-consultation-index'
  version: 2
  worldReference: WorldReferenceV1
  release: { version: number; label: string; contentHash: string }
  opportunities: {
    storySources: GameProductionSourceOptionsV1['storySources']
    characters: GameProductionSourceOptionsV1['characters']
    storyArcs: GameProductionSourceOptionsV1['storyArcs']
    historicalTimelineEvents: GameProductionSourceOptionsV1['storyArcs']
  }
 ...
 */
export function resolveGameProductionWorldCompilationDescriptorsV2(input: {
  ...
  if (selectedKeys.size !== input.selection.resourceKeys.length) {
    throw new Error('[game-production-world-source] selection resourceKeys 重复')
  }
  ...
  if ([...selectedKeys].some(key => !descriptorByKey.has(key))) {
    throw new Error('[game-production-world-source] selection 含冻结世界之外的资源')
  }
 ...
 */
export async function loadGameProductionConsultationSourceV2(input: {
  scope: WorkspaceScope
  worldReleaseId: number
}): Promise<GameProductionConsultationSourceV2> {
  ...
  const options: GameProductionSourceOptionsV1 = {
...
export async function loadGameProductionWorldSourceCatalogV2(input: {
  scope: WorkspaceScope
  worldReleaseId: number
  /** Formal production must pass the author-confirmed selection. Omitting it
   * is retained only for isolated compiler fixtures and diagnostics. */
  selection?: ProductWorldSourceSelectionV1
  ...
  const descriptors = input.selection
