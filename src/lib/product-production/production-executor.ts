...
function textSystem(
  ...
  if (isTextAdventureQuestScriptModelTask(taskKey)) {
    ...
    return `${common}\n你是任务脚本工程师。你不设计新故事、不修改上游阶段或目标，也不直接写运行状态；你的职责是把已采纳的主线、支线和区域事件计划逐项翻译为受控的检查参数、时间成本与三档结算文本。` +
      ...
      '补充任务的每个 sideQuestScripts[].stages[] 和 ambientEventScripts[].stages[] 必须恰好只有 stageKey、actionKind、abilityKey、difficulty、costlySuccessFloor、timeCostMinutes、successText、costlySuccessText、failureText 这 9 个字段。补充任务的失败文案字段名是 failureText，不是主线解法使用的 failureForwardText；严禁混用或同时输出两者。' +
