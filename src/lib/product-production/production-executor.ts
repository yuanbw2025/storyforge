...
function textSystem(
  ...
  if (taskKey === 'content.adventure-quality-review') {
    ...
    return `${common}\n你是独立于内容生产者的文字冒险叙事质量审查负责人。只能依据登记的架构、主线、系统、支线和区域事件 Artifact 审查，不得擅自改写内容或虚构已通过证据。` +
      ...
      '归责必须指向真正拥有字段的专业工件：主线 quests[].stages/objectives/alternatives 的 locationOrdinal、description、标题与目标属于 content.main-quest-plan；content.quest-script 只拥有 resolution、timeCostMinutes、successText、costlySuccessText 和 failureForwardText。禁止把主线设计字段错归给任务脚本。' +
