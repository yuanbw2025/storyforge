...
export async function readProductProductionRepairFeedback(input: AssembleContextInput): Promise<string> {
  const { scope, production, build } = await productionAndBuild(input)
  ...
  if (production.productType !== 'text-open-world') {
    ...
    const authorDraftJson = resolution?.action === 'author-edit'
      && typeof resolution.authorDraftJson === 'string' ? resolution.authorDraftJson : null
    ...
    if (!runs.length) return JSON.stringify({
    ...
    const latestAttempt = Math.max(0, ...events.map(event => (
      event.type === 'evidence.artifact.recorded' ? event.payload.attempt ?? 0 : 0
    )))
    const evidence = []
    for (const event of events) {
      if (event.type !== 'evidence.artifact.recorded' || event.payload.attempt !== latestAttempt) continue
      evidence.push({
        kind: event.payload.artifactKind,
        contentHash: event.payload.contentHash,
        content: await readAgentRunArtifactExactV1({
    ...
    return JSON.stringify({
      schema: 'storyforge.product-production.repair-feedback', version: 1,
      taskKey: input.productProductionTaskKey, authorRepairNote, authorDraftJson,
      previous: {
        runId: snapshot.run.id, contractHash: snapshot.run.contractHash,
        controlEpoch: boundary.controlEpoch, attempt: latestAttempt, evidence,
      },
    })
  }
  const failureState = (() => {
    try {
      const parsed: unknown = JSON.parse(build.failureJson)
      return parsed && typeof parsed === 'object' && !Array.isArray(parsed)
        ? parsed as Record<string, unknown> : {}
    } catch { return {} }
  })()
  // A normal first-pass task has no repair authority. Its registered optional
  // source must resolve to an empty packet instead of turning every fresh
  // production into a recovery-provenance failure.
  const failureTaskKey = typeof failureState.blockerKey === 'string'
    ? failureState.blockerKey
    : typeof failureState.taskKey === 'string' ? failureState.taskKey : null
  if (failureTaskKey == null || failureTaskKey !== input.productProductionTaskKey) {
    return JSON.stringify({
      schema: 'storyforge.product-production.repair-feedback', version: 1,
      authorRepairNote: null, authorDraftJson: null, previous: null,
    })
  }
