...
export async function createProductProductionPlanV3(input: {
  ...
  const textAdventureOutputWeights: Record<string, number> = {
    ...
    'content.narrative-arc-plan': 0.055,
    'content.main-quest-plan': 0.065,
    'content.quest-script': 0.04,
    // Scene prose is the player-visible product, not scaffolding. Reserve 27%
    // of the full output envelope for the three independent act writers and
    // another 7.5% for their independent dialogue passes.
    'content.scene-script.act-1': 0.09,
    'content.scene-script.act-2': 0.09,
    'content.scene-script.act-3': 0.09,
    'content.dialogue-pass.act-1': 0.025,
    'content.dialogue-pass.act-2': 0.025,
    'content.dialogue-pass.act-3': 0.025,
    'content.adventure-side-quests': 0.045,
    'content.adventure-ambient-events': 0.035,
    'content.adventure-quality-review': 0.03,
    'media.requirements': 0.025,
    'media.visual-quality-review': 0.015,
