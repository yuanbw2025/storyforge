...
| TypeScript 生产源码 | 551 个文件 / 122597 行 | `tsconfig.json` |
| IndexedDB schema | v50 / 66 张 required tables | `schema.ts` / `REQUIRED_TABLES` |
| PROJECT_TABLES | 66 张表 | `project-tables.ts` |
| Prompt 主线 | 61 个 moduleKey / 206 条内置模板 | `PromptModuleKey` / `prompt-seeds*.ts` |
| CONTEXT_SOURCES | 48 个上下文源 | `context-sources.ts` |
