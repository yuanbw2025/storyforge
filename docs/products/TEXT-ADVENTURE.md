...
- **AI 生产受治理**：正式计划登记文案主管、叙事骨架、主线、通用系统、支线、区域事件、独立质量审查和美术需求八个有界模型任务；输入只来自 Brief 与已登记上游工件，输出先过严格 schema，再进入 durable run、artifact、receipt 与预算证据链。质量审查读取登记的确定性审查投影，完整保留选择图、目标节点开场、任务地点/结果和角色/资源系统证据，避免大体积 Artifact 在模型窗口内被整段裁掉后产生“未登记”的假缺陷。玩家身份与占位角色泄漏另由确定性 RuntimePackage 质量门核验。
- **叙事质量会阻断并引导修复**：独立审查以因果、能动性、路线差异、节奏、铺垫回收、人物动机和情绪触达七项评分；阻塞问题或低分会保留审查 Artifact 并阻止 RuntimePackage 装配，不能以提示词自证通过。作者点击“修正后重试”后，系统按 blocking issue 指向的 Artifact 计算依赖闭包，只让对应专用 Agent 读取登记的上一轮修复反馈并重新生成，未受影响工件只补签新 epoch 回执。
...
- 质量审查不通过时已支持“保留问题 → 作者确认重试 → 按 Artifact 依赖闭包有界重生成 → 重新审查”；仍需增加逐工件修复前后差异预览、选择性采纳与更细的作者锁定能力。
