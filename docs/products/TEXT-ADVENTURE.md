...
> 层级：L2 · 版本：1.6.0 · 生效：2026-09-21
> 状态：现行产品契约与工程基座；专业生产与旗舰交付以 [`text-adventure-production/README.md`](./text-adventure-production/README.md) 为唯一施工入口。

> **分支说明：** 本轮通过 `feat/text-adventure-mainline-integration` 把文字冒险能力集成到经验证的最新主干；该分支只承载 `text-adventure` 的产品契约、生产、媒资、Build/Release、运行、播放器、测试与迁移，其他产品不得把它当作开发基线。合并后 `main` 才是新的共享基线，后续文字冒险施工仍须从该基线建立独立功能分支，不直接在 `main` 开发。
...
专业生产底座已登记 19 个独立岗位，每个岗位只挂载自己的核心 Skill；叙事弧、结局路线、主支线、任务脚本、分场正文、对白、连续性、媒资、自动游玩和真人试玩都有独立 Run Contract、严格 Artifact 与确定性装配边界。完整主线不再压成单任务单阶段；结局资格由互斥、完备且可穷举验证的持久决定条件裁决，不再由编译器按数组前缀猜测。角色系统 Artifact 也必须逐字覆盖冻结 Brief 的全部属性、技能和装备槽。任务路线、技能成长、物品操作、装备、地图、旅程、存档、分支和结局回顾已进入正式播放器与事件权威链路。

技术黄金路径已经在受控 Provider 输出下闭合：同一次正式 Production 从冻结世界与授权 Brief 进入专业 DAG，完成剧情/任务/媒资工件、装配、质量门、可玩 Preview、主线通关、不可变发布和含媒资分发包导出；该同一分发包随后原样进入隔离的新 Work，复验 Release/hash/媒资并再次从开局通到结局。独立浏览器 E2E 另验证实际文件下载、全新 Work 上传、刷新恢复、存档分支、双结局和删除生命周期。受控输出只用于隔离外部模型波动，这一结论是“工程路径可重复跑通”，不是“真实内容质量已达社区推荐”。
