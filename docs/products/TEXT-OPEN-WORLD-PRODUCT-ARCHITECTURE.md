...
真实库存产生136个受治理Scene，超过早期127项原型上限。当前硬上限提升为159；P9最多使用159项隔离Scene请求、一项共享表现请求和一次明确片段修复，共161次授权。完整DAG建议187次模型调用、产品硬上限200次，token按199份重新分配，其中P8F/P9/P10分别占17/26/18份。精确结构化生产Context必须全量进入任务预算，不能通过截断JSON伪装成功。上述数字是当前受治理容量，不代表真人已经验证3～5小时体感；时长、重复感和叙事质量仍由G7-11与G7-13验收。
