...
- 平局按“高先攻优先、玩家优先、稳定参与者键”决定，回放不依赖对象遍历顺序或AI描述；跳过已战败参与者，具体伤害与战败标记由G2-25负责；
...
图中的`prepared`目前由战前检查、确认和重试检查点表达，不另存一个可漂移的战斗阶段；`aborted-by-recovery`仍由既有复活/分支生命周期表达，实际回合操作已由G2-24完成，数值、随机证据和胜利领奖归G2-25。
