...
现行vNext包边界已实现于 `src/lib/types/text-open-world-runtime.ts` 和 `src/lib/open-world/runtime-package.ts`。当前完成的是严格包络、来源、校准、模块依赖、媒资、质量和兼容解析；各模块payload的领域Schema按G1-02继续实现，不能把“包可解析”误报成玩法已完成。
