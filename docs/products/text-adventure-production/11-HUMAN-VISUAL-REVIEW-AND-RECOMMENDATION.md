...
隔离浏览器已经覆盖“退回图片 → 冻结父 Build 失败回执 → 真实 file input 上传单图 → 派生不可变子 Build → 暂停/恢复重审 → 新 hash 全部接受”。测试同时验证父 Build 失败回执不迁移，子 Build 只生成自己的通过回执，未修改图片保留 Blob 字节但重绑子 Build `assetKey`。同图多用途导致内容 hash 重复时，逐项决定仍完整保存在 evidence，通用 receipt 的输入 hash 集合则确定性去重，避免合法复用被错误拒绝。
