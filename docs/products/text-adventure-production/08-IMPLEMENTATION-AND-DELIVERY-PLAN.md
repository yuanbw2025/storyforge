...
状态：专业依赖拓扑、来源决策专用闸门、三幕有界 Scene Writer、三幕 Dialogue Editor、确定性装配、连续性审校、自动游玩、质量门和试玩策略已接通。工作台已区分来源可接受补充与阻断冲突，并保存作者命令证据；总任务与各泳道展示真实 receipt 进度，文字冒险媒资已经按单项 task 显示生成数。作者锁定/差异仍需继续完善。
...
状态：部分实现。独立视觉圣经 Artifact、角色锚点 hash 绑定作者确认和逐项媒资 durable task 已闭合；单项上传/替换/锁定/重生成、审图和完整生命周期 E2E 尚未闭合。

- 已实现：确定性视觉圣经编译、商业候选角色锚点作者闸门、稳定计划内 assetKey 和逐项生成/有界纯文字降级。
- 待实现：作者上传/替换/锁定/单项重生成、独立审图与清单-素材-引用三向核查。
