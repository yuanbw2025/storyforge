...
状态：底座已实现。独立视觉圣经 Artifact、角色锚点 hash 绑定作者确认、逐项媒资 durable task，以及作者上传/替换/锁定/解锁/单项重生成的新 Build 派生链已闭合；需求—Artifact—Runtime 的确定性三向审计、独立 Visual QA Director 多模态审图和作者逐图人工回执均已进入正式发布硬门。本地候选包“上传文件 → 离线复验 → 新 Work 原子导入 → 正式 Release 双结局 → 刷新恢复”浏览器 E2E 已闭合；作者退回图片、真实 file input 单图替换、暂停恢复、新 Build 重新装配和新 hash 逐图复验也已有隔离浏览器 E2E。旧两图商业 Build 还可在同一 Production 取消后生成只改变媒资数量与预算的 Brief 修订候选，经作者保存、再次授权形成十二图子 Build；来源 hash 与旧工件保持不变。同一真实旗舰的下载文件、全新 Work 再上传、删除副本和共享 Blob 引用核查仍须在批次 G 执行。
...
状态：真实生产进行中，尚未形成可试玩 Build。隔离世界 `潮钟群岛旗舰来源世界 v4`、商业候选 Brief、冻结 SourcePlan 和来源私域补充决定已经建立；已完成 38/47 项 durable task，叙事、任务、对白、连续性审查、美术需求与视觉圣经均已有真实候选。当前旧 Brief 仅计划 2 张图，低于商业关键插图 12 张底线，系统已在 `media.anchor-author-gate` 禁止确认和 Provider 消耗；作者取消旧 Build 后，工作台会在同一 Production 提供“生成 12 图修订 Brief → 审查差异 → 保存 r2 → 再次授权”的显式路径，不能绕过闸门或用既有工件冒充可发布成品。
