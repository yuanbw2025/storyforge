...
作者在角色锚点闸门选择“拒绝并取消”时，取消 Build 必须保留命令、原 blocker、Plan 和全部工件证据，并立即允许同一条 `visual`-only 恢复路径；不得出现界面要求“取消后修订”、状态机却只允许从可预览 Build 演化的死路。普通 `stop` 产生的取消 Build 不具备该权限，只有可验证的 `media.anchor-author-gate` 拒绝链才能派生修订 Brief。
