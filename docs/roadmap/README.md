...
|---:|---|---|---|---|---|
| 0 | **GOV-1 架构、质量与发布治理** | 保证扩展前的架构、文档、供应链和发布结论可信 | AI Manual 假绿、参考分析绕过 `adopt()`、架构守卫盲区、事实账本扩展点、依赖漏洞、版本/Blueprint 漂移；`AUDIT-6`、`AUDIT-7`、`HEALTH-1`、`HEALTH-4`、`HEALTH-5` 的治理部分 | 旁路清零或正式登记；关键运行时代码有真实门槛；发布与文档自动对齐 | **READY；先做** |
| 1 | **INV-1 角色物品与状态账本** | 让主角、配角、NPC 拥有真实且可追溯的个人背包 | `INVENTORY-1`、`QUICKWIN-3`、角色状态同步、owner-less 历史数据迁移、按角色一致性 | 持有人、转移、提取范围、角色切换、状态投影、删除/合并、导入导出和迁移一次收口 | **READY；GOV-1 P1 收口后** |
...
1. GOV-1 第一阶段：修复本次架构审查发现的治理阻塞，并同步真实版本/规模。
