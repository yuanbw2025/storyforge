...
|---|---|
| 首版工作包 | 121 |
| 已完成 | 76（G0、G1、G2及G3全部完成，G4完成7项） |
| 产品总进度 | 76 / 121（62.8%） |
| G1～G7业务功能进度 | 66 / 111（59.5%） |
| 当前阶段 | G4 完整玩家端 |
| G0阶段进度 | 10 / 10（100%） |
| G1阶段进度 | 13 / 13（100%） |
| G2阶段进度 | 28 / 28（100%） |
...
| TOW-G4-08 | READY | 背包、物品详情、装备和比较UI | G2-06～G2-08 | 分类、使用、装备、出售、丢弃和关键保护反馈正确 |
| TOW-G4-09 | QUEUED | 逐回合战斗UI | G2-22～G2-25 | 四类操作、技能/道具不可用原因、日志、奖励、失败恢复完整；开工前闭合主动状态/资源/恢复技能编译、被动Effect、scalingAttribute与skillPower结算、状态生命周期/叠层/修正及cooldownTurns计数口径和1/2回合边界测试 |
