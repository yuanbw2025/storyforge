...
|---|---|
| 首版工作包 | 121 |
| 已完成 | 3（独立分支、架构文档、36项产品决策） |
| 产品总进度 | 3 / 121（2.5%） |
| 业务代码进度 | 0% |
| 当前阶段 | G0 现状复核与开工冻结 |
| 首个可开工项 | `TOW-G0-04` |
| 当前阻塞项 | 无；`TOW-G0-05`需在世界引擎出口稳定后完成 |
...
| ID | 状态 | 工作包 | 依赖 | 完成判据 |
|---|---|---|---|---|
| TOW-G0-01 | DONE | 建立文字开放世界专属分支 | 无 | 分支 `feat/text-open-world-product-architecture` 独立存在且工作树干净 |
| TOW-G0-02 | DONE | 建立整体产品、叙事子系统和愿景演进三层文档 | 无 | 三份文档进入权威清单、上下文路由和文档检查器 |
| TOW-G0-03 | DONE | 冻结36项首版产品决策 | G0-02 | 总架构第34节、叙事规格和愿景记录一致 |
| TOW-G0-04 | READY | 审计当前文字开放世界完整关联闭包 | 用户开始指令 | 形成入口→服务→类型→三注册表→表→测试→下游清单，不把旧文件名当能力事实 |
| TOW-G0-05 | QUEUED | 核验世界引擎正式读取出口 | 世界引擎清理/开发结果 | `describe/search/read WorldRelease`、版本、Hash、权限和缺口语义有代码证据 |
| TOW-G0-06 | QUEUED | 建立现有能力复用/改造/删除矩阵 | G0-04、G0-05 | 每个现有模块有owner、消费者、处理方式、迁移和反例 |
| TOW-G0-07 | QUEUED | 冻结首批校准参数登记方式 | G0-04 | 时长默认、关系阈值、不可放弃UI、随机正文时机、AI预算进入配置或决策ID |
