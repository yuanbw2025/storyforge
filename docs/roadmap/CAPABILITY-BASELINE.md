...
> 版本：1.6.12 · 基线：2026-09-13 文字开放世界G6-07受治理叙事导演建议 · 权威层级：L2
...
| E-OPENWORLD-01 | partial | 已在统一Production/Build/ProductRelease和ProductRuntime事件底座上完成G1运行治理、G2确定性玩法闭环、G3 P0～P10/V1～V3/QA内容生产编译器、G4完整玩家端、G5完整Creator生产/发布/更新阶段及G6-01～G6-07运行时AI合同、按需Context、自由输入采用、NPC对白、终态结果演绎、地区任务包装和叙事导演建议。成长、任务、地图/快旅、时间天气、NPC日程/生死/关系、战斗、制作、经济、地区演化、受限发牌、动态任务、随机事件与知识历程均可重放；Creator双来源生产、修改、媒资、质量、发布、版本维护、便携恢复和删除已闭合。Direction默认不调用模型；玩家按作品授权后，也只在代码已判定非Blank且存在至少两个合法非保护候选时运行一次，AI只能建议现有候选或不偏置，Blank、密度、冷却、冲突、容量、强度、主线保护与Effect继续由代码决定。Intent、Dialogue、Expression、Quest Packaging与Direction均保存完整V3 Harness证据且无模型状态写权限；模型失败、中断或stale不阻断确定性玩法，待结算Director命令恢复时不重发模型。临时对白不等于长期记忆，结构门不等于已证明文风、防改写式剧透、结果零幻觉、地区文案零重复或导演节奏质量；旧Release继续不可变兼容 | 长期最小记忆及统一模型故障/成本体验尚未接入；G6-08～G6-10及G7盐脊真人验收、压力/质量校准与旧入口收口仍未完成 |
