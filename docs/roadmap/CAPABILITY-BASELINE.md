...
> 版本：1.6.33 · 基线：2026-09-21 文字开放世界G7-15全量自动化交付门 · 权威层级：L2
...
| E-OPENWORLD-01 | partial | 已在统一Production/Build/ProductRelease和ProductRuntime事件底座上完成G1运行治理、G2确定性玩法闭环、G3 P0～P10/V1～V3/QA内容生产编译器、G4完整玩家端、G5完整Creator生产/发布/更新阶段及完整G6运行时AI。成长、任务、地图/快旅、时间天气、NPC日程/生死/关系、战斗、制作、经济、地区演化、受限发牌、动态任务、随机事件与知识历程均可重放；Creator双来源生产、修改、媒资、质量、发布、版本维护、便携恢复和删除已闭合。六类运行时AI具有合同、按需Context、自由输入、NPC对白、结果演绎、地区包装、导演建议、长期记忆、统一失败/成本边界及8项版本化评测；每Run最多一次调用，未知结果不隐藏重发，固定玩法始终可用。G7-01至G7-04已完成盐脊独立语义WorldRelease、2区10点程序地图、7段严格主线、两条结局路线、两类重要故事及8固定普通任务/5模板×3变体/16原创事件。G7-05以20级曲线和主线精确1→5级经验预算闭合成长，5项验收技能覆盖攻击/状态/恢复；3敌人家族覆盖3强度、两区和1～5级；至少8装备、4消耗品、5材料、2关键物、4～6配方与两区商店均进入RuntimePackage真实Action/Effect。G7-06进一步闭合1张程序SVG地图、6个关键角色头像和10个逐地点背景；18个图片输出具Blob、来源、许可、权利、alt与完整文字降级，Presentation v3显式绑定媒资主体，玩家从不可变Build/Release显示背景和当前位置头像并在失败时保留文字体验，Location引用和Creator整组重生成链可验。G7-07把产品架构规定的17条完整玩家路径冻结为机器校验矩阵，每项绑定实际Vitest/Playwright证据，10项UI关键路径保留Chromium覆盖；制作→战斗用药、角色/势力重要线、同Release双结局分支、犯罪后主线可达均有新增跨系统实证。G7-08进一步将12条系统不变量冻结为五层测试矩阵；16×32步Effect属性序列和两组同Release固定seed双Session真实行动链证明资源有界、所有权唯一、普通内容后主线可达、命令幂等、Session隔离及最终重放Hash一致。G7-09以固定seed在双地区完成1,000批次真实Director演化并完整重跑，100周期跃迁仍受32周期单批追赶限制，任务容量、64条历史/指纹、地区归属和关键故事等待均保持有界且不进入模型/Agent循环。G7-10用96地点、180物品定义/177持有物、121任务和160事件贯穿真实Release、Session、投影、线性重放、检查点及项目导入导出；本机Chromium桌面与390×844移动视口通过大地图、长任务、大背包和最低可访问性预算门，且明确不冒充未实测硬件认证。G7-11已建成当前Build绑定的真实模型校准门：独立grader必须与生产生成器使用不同Provider/Model，代码量化叙事评分、模板重复、主线及可选库存时长、实际调用/token/时延和冻结目录价估算，并以不可变回执阻断漂移或失败发布；自动测试只验证协议，真实通过仍等待作者显式调用。G7-12进一步以12项版本化门闭合当前v10项目备份、质量回执、Release媒资字节、Session/Event/Checkpoint/分支、旧Release留存、显式兼容迁移和Work/Session删除；回执、媒资、检查点或迁移证据篡改均失败关闭，删除产品Work不删除World拥有的WorldRelease。G7-13已建成不可代签的真人双结局验收设施：2～6条当前Build完成路线至少覆盖两个结局和六类核心循环，真实运行时AI请求/响应及等待进入账本，创作者逐路线记录主动时长并完成十项体验评价、费用来源声明和四项真人确认；低分与未解决问题只形成修复态，问题集变化使旧结论失效。G7-14进一步把正式Release真人问题、局部修复授权、直接后继Release、新版通过态试玩、逐问题/低分复测和旧档固定或显式兼容迁移收口为单一人类证据链。G7-15已删除通用节点旧四模块编译和Adapter注册，通用执行器失败关闭；新包默认纯vNext，旧Agent契约与旧单页Player入口由机器门防复活，历史旧包只能从不可变Release进入兼容播放器；17项隔离Chromium和全仓699文件/3554项测试、84.49%覆盖率、生产构建与bundle闸门全部通过。当前136个Scene受159项硬上限保护，完整DAG建议187次调用、产品硬上限200次、token权重202，精确结构化Context不得截断 | 自动化全量交付门已完成；真实独立模型通过回执、真实美术质量、作者双结局完整游玩回执及正式发布更新仍未完成，旧生产/预览双轨已完成工程退场 |
