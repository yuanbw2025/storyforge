...
当前应用只支持唯一 schema v1，不包含任何前代迁移链。任何 schema 或 owner 改动至少验证：

1. 新建数据库；
2. 非当前数据库版本被明确拒绝且不被改写；
...
本次硬切换以前的版本不在支持集合中。未来若决定支持从某个当前发行版本升级，必须先明确支持窗口、单向迁移和退出条件，并为该次演进补齐上述证据；不得借此恢复已删除的前代 schema、字段 alias 或通用兼容层。测试不得触碰作者数据库，也不得依赖当前浏览器缓存掩盖版本问题。
