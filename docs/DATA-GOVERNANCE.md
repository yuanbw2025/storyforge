...
> 版本：1.5.0 · 生效：2026-09-04 · 权威层级：L1
...
当前应用写入 schema v3，并支持从已发布 v1、v2 原位纯加表升级到 v3；升级不改写已有记录。v3 新增的来源事实、因果边和改编决策由目标 Work 独占，并按冻结 source manifest 版本隔离。任何 schema 或 owner 改动至少验证：

1. 新建数据库；
2. v1/v2→v3 原位迁移保留作者记录，未来版本和不在支持窗口内的版本被明确拒绝且不被改写；
