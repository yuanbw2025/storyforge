...
- 绑定本地记忆工作区；绑定后不会自动写入，需手动检查、确认采纳或同步。
...
当前创作可靠性入口属于实验性社区观察能力。工程恢复、费用停止和可编辑交付已经完成；独立作者 A/B 与原预注册质量门由产品决策关闭，不再是待开发项，但历史结果没有因此变成通过。它不能被理解为“任何模型都能稳定生成完美长篇”，社区反馈将用于判断后续需要重开的具体问题。
...
- 清理浏览器数据可能删除项目。
- 建议定期导出 JSON 完整备份。
- 重要项目建议绑定本地记忆工作区，并在每轮外部编辑后手动检查和同步；它不会在后台自动覆盖文件。
