...
| TypeScript 生产源码 | 986 个文件 / 331072 行 | `tsconfig.json` |
| IndexedDB schema | v4 / 102 张 required tables | `schema.ts` / `REQUIRED_TABLES` |
| PROJECT_TABLES | 102 张表 | `project-tables.ts` |
| Prompt 主线 | 65 个 moduleKey / 210 条内置模板 | `PromptModuleKey` / `prompt-seeds*.ts` |
| CONTEXT_SOURCES | 88 个上下文源 | `context-sources.ts` |
| 写回治理 | 44 个通用 adopt target / 42 个领域扩展 | `adoption-schema.ts` |
<!-- project-metrics:end -->
