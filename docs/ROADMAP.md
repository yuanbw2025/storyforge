...
- **状态（2026-07-13）**：`codex/isolated-bugs-20260710` 已基于 v3.8.0 重放，待最终验证。所有系统 `outline.volume` seed 统一使用 `VOLUME_OUTLINE_PARAMETERS`，并修正扩展玄幻 / 历史包旧变量契约；已加回归测试防止题材包再次缺参数或引用 adapter 不传的变量。
...
- **状态（2026-07-13）**：`codex/isolated-bugs-20260710` 已基于 v3.8.0 重放，待最终验证。`loadProject(id)` 会把 DB 中查到的新项目 upsert 到 `projects` 列表，覆盖“已有项目列表非空但不含导入项目”的卡死场景。
...
- **状态（2026-07-13）**：`codex/cf-relation-save-autofill-20260710` 已基于 v3.8.0 完成自审与验证，待审查 / 合并。范围：当前项目过滤、关系图 props 化、保存反馈、非法端点提示；不改 schema。预览浏览器已实测新建关系、中文标签/描述自动保存、刷新后关系与中文标签仍存在。
...
- **状态（2026-07-13）**：`codex/cf-relation-save-autofill-20260710` 已基于 v3.8.0 完成自审与验证，待审查 / 合并。范围：仅在采纳 AI 抽取关系后通过 `adopt()` 追加双方角色卡 `relationships`；手动新增的默认占位关系不反写，避免用户编辑关系后角色卡残留错误摘要；编辑 / 删除反写另需设计。AI 写回、去重、双向/单向和连续多条导入由真实 IndexedDB 回归测试覆盖；无 API 配置时未冒充完成真实模型调用。
...
- **状态（2026-07-13）**：`codex/isolated-bugs-20260710` 已基于 v3.8.0 完成根因修正并通过 UI 复测。每个维度改为独立 draft：输入期间忽略父级旧值回灌，400ms 防抖、失焦和组件卸载都会提交最新值；保留 `CTextarea` 中文 IME 保护。自测曾发现旧版“仅 blur 提交 + 单一 activeKey”在连续编辑两个字段后会丢失前一个字段，现由组件行为测试和“连续输入两个字段 → 立即切到大纲 → 返回 → 刷新”真实路径共同锁定，两字段均完整保留且不重复。
