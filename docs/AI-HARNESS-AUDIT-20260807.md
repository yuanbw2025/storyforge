...
| 角色关系提取 | `CharacterRelationPanel.tsx`、`character-relationship-durable.ts` | HARNESS-60 后只提交 `character.relationships`；当前 World 的角色/关系和当前 Work 内同 World 大纲/正文只经 `assembleContext()` 读取，严格候选使用唯一精确角色名绑定 ID；确认后经 `adopt(characterRelations)` 与登记的角色关系摘要同步写回 | 候选/采纳意图 checkpoint、baseline CAS、八个持久化中断点恢复、拒绝/导入取消、多世界隔离和正式表终验已接入。旧 `useAIStream`、手拼 Prompt 与逐条 store 写入旁路已下线；人工 CRUD 保留 | A（HARNESS-60） |
| 重要地点提取 | `LocationPanel.tsx`、`location-extraction-durable.ts` | HARNESS-62 后只提交 `world-origin.locations`；当前 Work 已写正文和当前 World 地点只经 `assembleContext()` 读取。计划冻结章节顺序/Manifest/分块/提示词/地点 baseline，全部分块成功后才形成严格候选；作者勾选后经 `adopt(importantLocations)` 写回 | 每分块 checkpoint 保证只续跑未完成调用；模型结果不确定窗口禁止自动重试。候选/冻结选择恢复、八个采纳边界、导入取消、Work/World 隔离、terminal stale 与正式表终验已接入。旧逐章 `chat`、手拼 Context、内存候选和直接 `adopt()` 旁路已下线；人工 CRUD 保留 | A（HARNESS-62） |
| 物品栏提取 | `InventoryPanel.tsx`、`inventory-extraction-durable.ts` | HARNESS-63 后只提交 `prose.inventory-extraction`；当前 Work 正文/流水与当前 World 角色只经 `assembleContext()` 读取。计划冻结提取范围、章节/Manifest/分块、提示词、roster 与正式 baseline，全部分块成功后才形成严格候选 | 作者冻结选择后按 `itemLedger.replaceScope=['chapterId']` 逐章事务替换；空结果可明确清理目标章。每章写后 checkpoint，写入/检查点崩溃窗可回读收敛；模型不确定窗口不重试。唯一姓名 FK、八采纳边界、导入取消、作用域隔离和 terminal stale 已接入；旧先删后写旁路下线，人工 CRUD 保留 | A（HARNESS-63） |
| 故事年表提取 / 影响单事件重建 | `StoryTimelinePanel.tsx`、`story-timeline-extraction-durable.ts`、`impact-story-timeline-regeneration-durable.ts`、`ChapterEditor.tsx` | HARNESS-64 集合提取只经 `chapterContent → assembleContext()` 读取当前 Work 正文，确认后按章原子替换；HARNESS-79 复用同一 Skill，但只经 `chapterContent / storyTimelineTarget` 读取 H57 精确目标正文与既有事件，一次调用形成单事件严格候选 | H64 负责新增/删除/重排与空选择清理；H79 只在 H57 parent 和直接依赖 review proof fresh 时，经目标完整 CAS + `adopt(merge-diffs)` 更新 `storyTime / importance / description`，身份/标题/章节/顺序冻结。两路均有候选恢复、采纳边界、导入取消、Work 隔离和 terminal stale；人工 CRUD 保留 | A（HARNESS-64/79） |
| 正文局部编辑 / 查漏 | `FloatingToolbar.tsx`、`selection-edit-durable.ts` | HARNESS-65 后分别提交 `prose.selection-edit / prose.selection-check`；模型只经 `manualText → assembleContext()` 读取作者冻结选区，不读取整章。Prompt、Manifest、选区位置、选中文字和完整 HTML baseline 全部冻结 | 编辑结果先持久化候选，作者确认后以正文文本 hash + 精确 HTML hash 双 CAS 经 `adopt(chapters)` 写回并回读终验；查漏只形成只读报告。模型结果未知不重试，八采纳边界、写后恢复、导入取消、Work/章节隔离与 terminal stale 已接入；旧 `useAIStream` 和直接 `replaceSelection()` 接受旁路下线 | A（HARNESS-65） |
| 世界地图配置 | `WorldMapPanel.tsx`、`world-map-config-durable.ts` | HARNESS-66 后只提交 `world-origin.map-config`；模型只经 `worldview / geography / codex / locations → assembleContext()` 读取当前 World，目标节点名由治理层冻结进入 Prompt。严格配置先成为不可便携 durable 候选，作者确认后才以 exact-field CAS 经 `adopt(worldNodes.mapConfigJSON)` 定点写回 | 模型结果未知不重试；节点名、Context、Prompt、原配置、八采纳边界、写后恢复、导入取消、World/世界组/节点隔离及 terminal stale 已接入。旧组件直接 DB 扫描、手拼 Context、宽松裁剪/随机 seed 与 AI `updateNode()` 旁路下线；确定性 Voronoi 求解与人工参数编辑保留 | A（HARNESS-66） |
| 世界组七字段扩写 | `WorldGroupDetail.tsx`、`worldview-expand-durable.ts` | HARNESS-67 后只提交 `world-origin.worldview-expand`；模型只经 `manualText / worldGroups / storyCore / worldview → assembleContext()` 读取已保存目标草稿、当前 Work 和当前 World。exact-key 七字段先成为不可便携 durable 候选，确认前正式世界观零写入 | 作者确认后在同一事务中复核目标组和完整世界观 baseline，再经 `adopt(worldviews)` 原子写七字段。模型结果未知不重试；Context/Prompt/非目标字段、八采纳边界、首次创建、写后恢复、导入取消、World/Work/世界组隔离及 terminal stale 已接入。旧组件直调、手拼 Context、宽松 parser 与立即采纳旁路下线；人工世界组编辑保留 | A（HARNESS-67） |
| 多世界 AI 建议 | `WorldGroupOverview.tsx`、`world-suggest-durable.ts` | HARNESS-68 后只提交 `world-origin.world-suggest`；模型只经 `manualText / worldGroups / storyCore → assembleContext()` 读作者方向、当前 World 目录和当前 Work 故事核心。strict 2～4 项世界先成为不可便携整批 durable 候选，确认前 `worldGroups` 零写入 | 作者选择非空子集后，冻结选择/顺序并复核项目提示、完整世界目录/关系、故事核心、Context 和 Prompt，再于同一事务经 `FIELD_REGISTRY + AdoptionSchema + adopt(worldGroups)` 原子新增八字段记录。同名、未知模型结果、八采纳边界、写后恢复、导入取消、World/Work 隔离和 terminal stale 已接入。旧 `useAIStream`、内存候选、直接 `createGroup()` 和 `buildAllWorldsOverview()` 旁路下线；世界/关系人工 CRUD 保留 | A（HARNESS-68） |
| 世界宪法设定扫描 | `WorldConstitutionPanel.tsx`、`constitution-extraction-durable.ts`、`setting-assertions.ts` | HARNESS-69 后只提交 `world-origin.constitution-extract`；模型只经 `constitutionScanSources → assembleContext()` 读取当前 World/Work 的登记设定来源、主题和主体，且仅接受无损完整闭集。strict 0～80 项结果先成为不可便携整批 durable 候选，批次确认前 `temporalFacts` 零写入 | 作者批次确认后，来源/主体/完整事实 baseline/Context/Prompt 在事务内仍一致，才经登记 `fact-ledger` Adoption Extension 原子写入 `status=candidate`；每条仍需既有逐条确认/明确取代后才成为 Canon。未知结果、八采纳边界、导入取消、隔离和 terminal stale 已接入；旧组件直调、直接查库、宽松 parser 与立即写入旁路下线 | A（HARNESS-69） |
| Codex 词条拆分 | `CodexPanel.tsx`、`codex-extraction-durable.ts`、`codex/extraction.ts` | HARNESS-70 后只提交 `world-origin.codex-extract`；模型只经 `manualText / codexExtractionBaseline → assembleContext()` 读取作者来源、目标分类/schema 与同世界组既有词条。长来源逐分块 checkpoint，全部调用后形成 strict 0～200 项不可便携候选，确认前 `codexEntries` 零写入 | 作者子集与顺序冻结后不可改选/取消；分类/schema/完整既有词条 baseline/Context/Prompt 在事务内仍一致时，才经 `FIELD_REGISTRY + AdoptionSchema + adopt(codexEntries)` 原子新增。零候选、未知结果、八采纳边界、导入取消、World/Work/世界组/分类隔离和 terminal stale 已接入；旧组件直调、组件 Context、宽松 parser 与直接写入旁路下线 | A（HARNESS-70） |
| 伏笔 AI 建议 | `ForeshadowPanel.tsx`、`foreshadow-suggestions-durable.ts`、`foreshadow/suggestions.ts` | HARNESS-72 后只提交 `outline.foreshadow-suggestions`；模型只经 Canon、世界、故事、角色、规则、历史、地点和 `foreshadowSuggestionBaseline → assembleContext()` 读取当前 Work，一次调用形成 strict 0～12 项不可便携候选，确认前 `foreshadows` 零写入 | 作者子集冻结后不可改选/取消；项目、完整伏笔 baseline、上游 Context 与 Prompt 在事务内仍一致时，才经 `FIELD_REGISTRY + AdoptionSchema + adopt(foreshadows)` 原子新增系统固定为 `planned` 的记录。空结果、未知结果、八采纳边界、导入取消、Work 隔离和 terminal stale 已接入；旧双模型直调、自由文本二次解析、内存候选与逐条写入旁路下线，人工 CRUD 保留 | A（HARNESS-72） |
| 历史考据 / 头脑风暴 | `HistoryPanel.tsx`、`useHistoryAI.ts`、`history-agent-durable.ts`、`history/agent-baseline.ts` | HARNESS-73 后按模式提交 `world-origin.history-consult / history-storm`；模型只经 `worldview / historyAgentBaseline → assembleContext()` 读取当前 World/世界组、已保存历史总述/纪年和精确事件或关键词，一次调用形成严格 Markdown 持久候选，确认前正式条目零写入 | 目标表/ID、来源、原结果字段 presence/value、Context/Prompt 全冻结；确认后只以 CAS 经 `adopt()` 替换 `aiConsult` 或 `aiBrainstorm`，另一路结果不受影响。未知窗口、八采纳边界、目标删除、导入取消、World/Work/世界组隔离和 terminal stale 已接入；旧两路 `useAIStream`、组件 Context/即时写回旁路下线，人工 CRUD 保留 | A（HARNESS-73） |
| 参考分析总结 / 角色聚合 | `AnalysisReportViewer.tsx`、`useReferenceDerivedAI.ts`、`reference-derived-durable.ts`、`reference-analysis/derived-agent-*` | HARNESS-74 后按模式提交 `inspiration.reference-summary / reference-characters`；模型只经 `referenceDerivedBaseline → assembleContext()` 读取当前 Work 的精确参考、精确分析版本、分块派生输入和来源声明，一次调用形成 strict JSON 持久候选，确认前版本字段与参考投影零写入 | 参考/版本 ID、来源 baseline、原版本及投影字段 presence/value、Context/Prompt 全冻结；确认后先经 CAS + `adopt(referenceAnalysisRuns)` 写版本字段，仅 active 版本再 `adopt(references)` 同步兼容投影。未知窗口、十个采纳边界、目标删除、导入取消、Work/版本隔离和 terminal stale 已接入；旧两处组件 `chat()`、内存输出和即时写回旁路下线 | A（HARNESS-74） |
| Prompt 示例辅助生成 | `PromptExamplesEditor.tsx`、`PromptTemplateEditor.tsx`、Prompt store | HARNESS-75 复核后归 `authoring-draft` 辅助入口：模型只读当前未保存 Prompt 草稿，不经项目 Context；输出只调用父组件 `onChange` 进入内存 draft，生成后 `promptTemplates` 与 Agent ledger 均零写 | 作者必须另点顶部「保存」才写 `PROJECT_TABLES` 已登记的全局、本机、不可项目导出的 `promptTemplates`；未保存离开即丢弃，系统模板只读，配置缺失零调用。它不是 Canon 写入，建立 durable Run 反而会错误改变草稿生命周期 | A（HARNESS-75 auxiliary） |
...
| 反向反馈 | `impact-analysis.ts`、`impact-remediation-plan.ts`、`impact-remediation-replan.ts`、`impact-patch-durable.ts`、`impact-remediation-durable.ts`、`impact-review-durable.ts`、`impact-handoff.ts`、`impact-handoff-durable.ts`、`impact-manual-correction-durable.ts`、`impact-post-correction-replan-durable.ts`、`impact-dependency-readiness.ts`、`impact-downstream-policy.ts`、`impact-downstream-schedule.ts`、`impact-generative-slot.ts`、`impact-outline-regeneration-durable.ts`、`impact-story-timeline-regeneration-durable.ts`、`ChapterEditor.tsx`、`WorkspacePage.tsx`、`R-HARNESS44-impact-patch-durable`、`R-HARNESS46-impact-remediation-plan`、`R-HARNESS47-impact-remediation-durable`、`R-HARNESS49-impact-remediation-replan`、`R-HARNESS50-impact-review-durable`、`R-HARNESS52-impact-handoff`、`R-HARNESS54-impact-handoff-validation`、`R-HARNESS55-impact-target-validation`、`R-HARNESS55-impact-target-ui`、`R-HARNESS56-impact-manual-correction`、`R-HARNESS57-impact-post-correction-replan`、`R-HARNESS58-impact-post-correction-remediation`、`R-HARNESS77-impact-outline-regeneration`、`R-HARNESS79-impact-story-timeline-regeneration`、`R-HARNESS80-impact-downstream-schedule`、`R-HARNESS81-impact-downstream-policy`、`R-AUDIT6-chapter-editor-toolbar` | HARNESS-43～58 建立影响图、作者复核、可信人工修正/current replan 与确定性 child；HARNESS-77/78 为后续 `outlineNodes.summary` 建立带依赖 proof 的生成式 child；HARNESS-79 扩展到精确既有年表事件；HARNESS-80 收口 canonical 五态 schedule/共享生成 slot；HARNESS-81 为全部现有 H57 节点冻结 exact executor policy | 两个生成式目标精确绑定 parent/plan/item、依赖 proof、Context/Prompt/目标 baseline；摘要/检索走 H58，其它正文、事实、来源及耦合/集合派生明确走作者复核和现有精确人工面板。未知或错配节点不再静默降级，而是停止调度 | A（受限 patch、计划、复核/交接/精确落点、人工保存证明、current replan、确定性 child、两个生成式目标、跨类型调度与全部当前目标政策）；D（无 durable 动作的临时计划持久化与真实模型质量评测仍缺） |
...
| 离线评测 | `NS-0`、NS/AGENT/PIPELINE 回归、H4 40+20 runner、H82～H85 真实模型证据 | 有 development/held-out fixture、预算、事实/约束/泄漏/证据指标；H83/H84 闭合协议纠错与 taxonomy 边界，H85 又把已验真证据判类拆成独立、逐调用记账、自包含且可恢复的 checkpoint，429 不再自动盲重试 | Agnes H85 完成 40/40 并把 precision/recall 提升到 78.8%/81.3%，但 precision 仍未达门，且两次历史 429 缺 usage 回执；Doubao 文本模型仍被真实 `403 AccountOverdueError` 阻断。held-out 保持锁定，仍缺可靠 verifier、真实 generator 主链 A/B、人工修改量/完成率/p95 与独立人工复核 | B（工程、两阶段恢复与真实负面证据），D（发布质量证据） |
...
| 下游目标若默认扩模型会扩大写入 | HARNESS-81 证明 H40 的故事线候选跨三表耦合，H20/H63 的状态/物品写入按整章集合替换，而 H57 对这些领域只持有单记录 item；H55 已为所有人工类型提供精确现有面板落点 | 事实 | 当前全部 H57 类型已有闭集执行政策；耦合/集合类型刻意走人工链，未来若增加生成式集合重建必须另行冻结删除、新增、范围 CAS 和终验，不能由单记录调度隐式授权 |
...
| 反馈 | stale + 后续章节提示；HARNESS-44～58 有受限 patch、复核、可信人工修正/current replan 和确定性 child；HARNESS-77～80 有两个生成式 child、依赖门和跨类型 schedule；HARNESS-81 对全部现有节点冻结执行器政策 | 新生成类型仍须一次只增加一个，并保持作者确认、目标/集合 CAS 和终态 verifier | 当前类型均可完成或进入精确人工面板；正文级重跑和集合级自动重建刻意不授权 | H4，中高 |
...
| 评测 | 模块级 NS/AGENT/PIPELINE、H4 40+20 基座；H82/H83 已取得两款真实文本模型 40/40 但未过质量门的 token/成本/延迟证据 | 可靠独立 verifier 先通过 development，再运行 sealed held-out；真实 generator 分步骤端到端、失败注入、回放等价、人工修改量/完成率/p95 | 协议 completion 已闭合，质量证据仍明确阻断发布；缺主链正向收益与人工 held-out | H0/H5，高 |
...
| `impact-analysis` 只提示 | HARNESS-44 至 50 已收口为影响图、受限大纲摘要 patch、确定性重建、stale/replan 和作者复核 receipt；继续保留安全默认 | 复核证据不等于 Canon 修正；不自动级联重写正文、事实权威状态或 locked 数据 |
