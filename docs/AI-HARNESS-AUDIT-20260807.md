...
| 故事年表提取 | `StoryTimelinePanel.tsx`、`story-timeline-extraction-durable.ts` | HARNESS-64 后只提交 `prose.story-timeline-extraction`；模型只经 `chapterContent → assembleContext()` 读取当前 Work 正文，正式年表 baseline 仅供治理层 CAS。计划冻结章节/Manifest/分块、提示词和完整正式年表，全部分块成功后才形成严格候选 | 候选按 `chapterId + title` 去重；作者冻结选择后按 `storyTimelineEvents.replaceScope=['chapterId']` 逐章事务替换并压缩 order。空选择可明确清理目标已写章；八采纳边界、写后 checkpoint 恢复、导入取消、Work 隔离和 terminal stale 已接入。旧组件直调、手拼 Context、宽松 parser 与先删后写旁路下线，人工 CRUD 保留 | A（HARNESS-64） |
...
| 反向反馈 | `impact-analysis.ts`、`impact-remediation-plan.ts`、`impact-remediation-replan.ts`、`impact-patch-durable.ts`、`impact-remediation-durable.ts`、`impact-review-durable.ts`、`impact-handoff.ts`、`impact-handoff-durable.ts`、`impact-manual-correction-durable.ts`、`impact-post-correction-replan-durable.ts`、`impact-outline-regeneration-durable.ts`、`impact-outline-regeneration.ts`、`ChapterEditor.tsx`、`WorkspacePage.tsx`、`R-HARNESS44-impact-patch-durable`、`R-HARNESS46-impact-remediation-plan`、`R-HARNESS47-impact-remediation-durable`、`R-HARNESS49-impact-remediation-replan`、`R-HARNESS50-impact-review-durable`、`R-HARNESS52-impact-handoff`、`R-HARNESS54-impact-handoff-validation`、`R-HARNESS55-impact-target-validation`、`R-HARNESS55-impact-target-ui`、`R-HARNESS56-impact-manual-correction`、`R-HARNESS57-impact-post-correction-replan`、`R-HARNESS58-impact-post-correction-remediation`、`R-HARNESS77-impact-outline-regeneration`、`R-AUDIT6-chapter-editor-toolbar` | HARNESS-43～57 建立影响图、作者复核、可信人工修正证明与 current replan；HARNESS-58 将 fresh H57 确定性余项绑定到 H47 child；HARNESS-77 为一个后续 `outlineNodes.summary` 建立重新装配当前 Context 的 H57 child 严格候选；HARNESS-78 要求目标直接上游先有同 plan 的 fresh `acknowledged` review proof | 当前同时有受控确定性派生重建和首个生成式目标类型。H77/78 精确绑定 H57 parent/plan/item、直接依赖 review、来源/Context/Prompt/目标 baseline；缺 proof 时零模型/零 Run，多个同类型目标逐项解锁，刷新恢复、八采纳边界、terminal stale 和导入生命周期已闭合。仍不自动级联或覆盖正文、事实、锁定数据和其它章纲字段 | A（受限 patch、计划、复核/交接/精确落点、人工保存证明、current replan、确定性 child 与带依赖证明的单目标生成式 child 主路径）；D（其它目标类型、跨类型通用调度、无 durable 动作的临时计划持久化与真实模型质量评测仍缺） |
...
| 下游反馈尚未形成通用闭环 | HARNESS-43 至 58 已形成影响图、受限作者 patch、作者复核、可信人工修正/current replan 和绑定 H57 的确定性派生 child；HARNESS-77/78 已为后续 `outlineNodes.summary` 建立带直接依赖 review proof 的生成式 child 候选与确认写回 | 事实 | 单一生成式目标类型已可审计、恢复、逐项按依赖解锁和终验；其它目标类型、跨类型调度、无 durable 动作的临时计划和通用重跑仍未完成 |
...
| 反馈 | stale + 后续章节提示；HARNESS-44/45 有作者手填受限摘要 patch；HARNESS-46～58 有计划、复核、可信人工修正/current replan 和确定性 child；HARNESS-77/78 有一个带直接依赖 proof 的后续章纲摘要生成式 H57 child | 扩展其它目标类型时仍须一次只增加一个，并保持作者确认、目标 CAS 和终态 verifier | 确定性余项与单一生成式章纲目标的依赖顺序已闭环，但其它类型/多字段/正文级重跑尚未编排 | H4，中高 |
