...
| 反向反馈 | `impact-analysis.ts`、`impact-remediation-plan.ts`、`impact-remediation-replan.ts`、`impact-patch-durable.ts`、`impact-remediation-durable.ts`、`impact-review-durable.ts`、`impact-handoff.ts`、`impact-handoff-durable.ts`、`ChapterEditor.tsx`、`WorkspacePage.tsx`、`R-HARNESS44-impact-patch-durable`、`R-HARNESS46-impact-remediation-plan`、`R-HARNESS47-impact-remediation-durable`、`R-HARNESS49-impact-remediation-replan`、`R-HARNESS50-impact-review-durable`、`R-HARNESS52-impact-handoff`、`R-HARNESS54-impact-handoff-validation`、`R-HARNESS55-impact-target-validation`、`R-HARNESS55-impact-target-ui`、`R-AUDIT6-chapter-editor-toolbar` | HARNESS-43 的影响图作为只读证据；HARNESS-44/45 提供受限大纲摘要 patch 主路径；HARNESS-46 生成绑定 graph/source hash 的处理计划；HARNESS-47 用零模型 durable Run 重建检索块/摘要；HARNESS-49 支持 stale 后重规划；HARNESS-50/51 记录并回放绑定 plan/item/source/graph hash 的作者复核 receipt；HARNESS-52/54 将 `needs-manual-action` 交接到既有入口，并在目标工作区重建当前图/计划、回放 Run/receipt 后才信任地址；HARNESS-53 在编辑器重挂载时恢复有效复核；HARNESS-55 再验证目标表/记录/作用域和当前模块，并让既有面板精确显露具体记录 | 当前只自动执行确定性派生缓存；作者复核 receipt 不写 Canon、不调用 `adopt()`，也不代表业务数据已修正；交接、精确落点和恢复均只读；无 durable 动作的临时计划、人工修正完成证明、自动级联重建、通用人工修正表单与依赖重跑仍缺 | A（受限 patch、计划、派生缓存、stale/replan、复核回放/恢复、durable 人工入口精确交接主路径）；D（通用反馈执行与重跑仍缺） |
...
| 下游反馈尚未形成通用闭环 | HARNESS-43 至 50 已形成影响图、受限 `outlineNodes.summary` patch、确定性重建、stale/replan 和作者复核 receipt；复核 Run 明确无 `adoption.committed` | 事实 | 第一条受限修订、派生缓存重建和复核证据已可追溯；事实、角色状态、物品、年表、故事线、其它大纲及正文仍缺受治理人工修正入口或依赖重跑，不能宣称通用反馈闭环已完成 |
...
| 反馈 | stale + 后续章节提示；HARNESS-44/45 有受限大纲摘要 patch；HARNESS-46/47/49 有处理计划、派生缓存执行和重规划；HARNESS-50 有作者复核 receipt | 更多受限 patch、人工修正入口、按依赖重跑 | 作者确认项目前只记录决定，不修改事实/状态/正文，也未编排依赖重跑 | H4，中高 |
