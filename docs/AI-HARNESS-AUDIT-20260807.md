...
| 离线评测 | `NS-0`、NS/AGENT/PIPELINE 回归、H4 40+20 runner、`HARNESS-82-REAL-MODEL-EVIDENCE-20260815.md` | 有 development/held-out fixture、预算、事实/约束/泄漏/证据指标；HARNESS-82 已用 Agnes 2.5 Flash 与 Doubao 1.5 Pro 文本取得可验签的真实 development 负面证据 | 两模型都未完成/通过 development，因此 held-out 保持锁定；仍缺可靠 verifier、真实 generator 分步骤主链 A/B、人工修改量/完成率/p95 与独立人工复核 | B（工程与真实负面证据），D（发布质量证据） |
...
| 评测 | 模块级 NS/AGENT/PIPELINE、H4 40+20 基座；H82 已取得两款真实文本模型未过 development 的 token/成本/延迟/质量证据 | 可靠独立 verifier 先通过 development，再运行 sealed held-out；真实 generator 分步骤端到端、失败注入、回放等价、人工修改量/完成率/p95 | 当前证据明确阻断发布，但仍缺主链正向收益与人工 held-out | H0/H5，高 |
