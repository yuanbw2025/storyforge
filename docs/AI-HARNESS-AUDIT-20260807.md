...
| 反向反馈 | `impact-analysis.ts`、`impact-remediation-plan.ts`、`impact-remediation-replan.ts`、`impact-patch-durable.ts`、`impact-remediation-durable.ts`、`impact-review-durable.ts`、`impact-handoff.ts`、`impact-handoff-durable.ts`、`impact-manual-correction-durable.ts`、`ChapterEditor.tsx`、`WorkspacePage.tsx`、`R-HARNESS44-impact-patch-durable`、`R-HARNESS46-impact-remediation-plan`、`R-HARNESS47-impact-remediation-durable`、`R-HARNESS49-impact-remediation-replan`、`R-HARNESS50-impact-review-durable`、`R-HARNESS52-impact-handoff`、`R-HARNESS54-impact-handoff-validation`、`R-HARNESS55-impact-target-validation`、`R-HARNESS55-impact-target-ui`、`R-HARNESS56-impact-manual-correction`、`R-AUDIT6-chapter-editor-toolbar` | HARNESS-43 的影响图作为只读证据；HARNESS-44/45 提供受限大纲摘要 patch 主路径；HARNESS-46 生成绑定 graph/source hash 的处理计划；HARNESS-47 用零模型 durable Run 重建检索块/摘要；HARNESS-49 支持 stale 后重规划；HARNESS-50/51 记录并回放绑定 plan/item/source/graph hash 的作者复核 receipt；HARNESS-52/54 将 `needs-manual-action` 交接到既有入口并验签；HARNESS-53 恢复当前复核；HARNESS-55 精确解析并显露目标；HARNESS-56 冻结目标 pre-state，只有既有面板保存造成同一正式记录业务状态变化后才签发完成 receipt | 当前只自动执行确定性派生缓存；人工修正完成证明本身零模型、零 Canon 写入且不等于旧影响项已解决；无 durable 动作的临时计划、修正完成后的新图分类、自动级联重建、通用人工修正表单与依赖重跑仍缺 | A（受限 patch、计划、派生缓存、stale/replan、复核/交接/精确落点/人工保存完成证明主路径）；D（修正后重规划与通用反馈执行/重跑仍缺） |
...
| 下游反馈尚未形成通用闭环 | HARNESS-43 至 56 已形成影响图、受限 `outlineNodes.summary` patch、确定性重建、stale/replan、作者复核、可信精确交接和人工保存前后状态 receipt；HARNESS-56 证明修改发生但不声明问题已解决 | 事实 | 第一条受限修订、派生缓存重建、复核与人工修改证据已可追溯；修正后当前图分类、生成式下游新 Run、通用目标重建和依赖重跑尚未完成，不能宣称通用反馈闭环已闭合 |
...
| 反馈 | stale + 后续章节提示；HARNESS-44/45 有受限大纲摘要 patch；HARNESS-46/47/49 有处理计划、派生缓存执行和重规划；HARNESS-50～56 有作者复核、可信精确交接与人工保存 pre/post receipt | 修正后当前 replan、更多受限 patch、生成式下游新 Run、按依赖重跑 | 已能证明既有面板的同一正式目标发生变化，但尚未证明旧影响已解决，也未编排生成式依赖重跑 | H4，中高 |
