...
| 反向反馈 | `impact-analysis.ts`、`impact-remediation-plan.ts`、`impact-remediation-replan.ts`、`impact-patch-durable.ts`、`impact-remediation-durable.ts`、`impact-review-durable.ts`、`impact-handoff.ts`、`ChapterEditor.tsx`、`WorkspacePage.tsx`、`R-HARNESS44-impact-patch-durable`、`R-HARNESS46-impact-remediation-plan`、`R-HARNESS47-impact-remediation-durable`、`R-HARNESS49-impact-remediation-replan`、`R-HARNESS50-impact-review-durable`、`R-HARNESS52-impact-handoff`、`R-AUDIT6-chapter-editor-toolbar` | HARNESS-43 的影响图作为只读证据；HARNESS-44/45 提供受限大纲摘要 patch 主路径；HARNESS-46 生成绑定 graph/source hash 的处理计划；HARNESS-47 用零模型 durable Run 重建检索块/摘要；HARNESS-49 支持 stale 后重规划；HARNESS-50/51 记录并回放绑定 plan/item/source/graph hash 的作者复核 receipt；HARNESS-52 将 `needs-manual-action` 交接到既有入口；HARNESS-53 在编辑器重挂载时从当前图/计划和 Run 账本恢复有效复核 | 当前只自动执行确定性派生缓存；作者复核 receipt 不写 Canon、不调用 `adopt()`，也不代表业务数据已修正；交接和恢复均只读；无 durable 动作的临时计划、自动级联重建、通用人工修正表单与依赖重跑仍缺 | A（受限 patch、计划、派生缓存、stale/replan、复核回放/恢复、人工入口交接主路径）；D（通用反馈执行与重跑仍缺） |
