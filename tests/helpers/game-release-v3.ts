 ...
 * provenance shape.
 */
export async function createFixtureGameReleaseManifestV3(input: {
  ...
  const portableReferenceBody: Omit<WorldReferenceV1, 'referenceHash'> = {
    ...
    manifestIdentity: {
      ...
      schemaHash: await hashCanonicalValue({ schema: 'storyforge.world-package', version: 2, semanticContract: 3 }),
    },
    capabilityIdentity: {
  ...
  const confirmedBrief: ConfirmedProductBriefV1 = await createConfirmedProductBriefV1({
    ...
    sourcePlan,
    briefRevision: 1,
    briefContentHash: await hashGameProductionValueV2(runtimePackage),
  ...
  const productionProvenance: GameReleaseManifestV3['productionProvenance'] = input.productionProvenance ?? {
    productionKey,
    buildNumber: 1,
  ...
  const identityBody: Omit<GameReleaseManifestV3, 'releaseIdentityHash' | 'lineage'> = {
    schema: 'storyforge.game-release', version: 3,
    productType: runtimePackage.productType,
  ...
  return createGameReleaseManifestV3({ runtimePackage, productionProvenance, sourceContracts, lineage })
}
