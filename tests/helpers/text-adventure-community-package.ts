...
export async function createTextAdventureCommunityPackageFixtureV1(input?: {
  ...
  const firstEndingIndex = baseRuntime.narrative.nodes.findIndex(node => node.kind === 'ending')
  baseRuntime.narrative.nodes.splice(firstEndingIndex, 0,
    { key: 'rescue.echo.1', kind: 'scene', title: '归航灯亮起', summary: '港口先听见船员的回应。', conditionJson: '{}', effectsJson: '[]', successorKeys: ['rescue.echo.2'] },
    { key: 'rescue.echo.2', kind: 'scene', title: '裂隙仍在', summary: '获救的代价随雾潮留在港外。', conditionJson: '{}', effectsJson: '[]', successorKeys: ['ending.rescue'] },
    { key: 'seal.echo.1', kind: 'scene', title: '城墙重获光明', summary: '港城居民确认屏障已经稳定。', conditionJson: '{}', effectsJson: '[]', successorKeys: ['seal.echo.2'] },
    { key: 'seal.echo.2', kind: 'scene', title: '海面归于黑暗', summary: '等待归航的人开始承担选择的代价。', conditionJson: '{}', effectsJson: '[]', successorKeys: ['ending.seal'] },
  )
  crossroads.successorKeys = ['rescue.echo.1', 'seal.echo.1']
  baseRuntime.narrative.beats.push(
    ...
    { beatKey: 'beat.rescue.echo.1', nodeKey: 'rescue.echo.1', kind: 'narration', speakerKey: null, text: '归航汽笛穿过雾墙，守钟人第一次放下紧握的铜铃。', order: 0 },
    { beatKey: 'beat.rescue.echo.2', nodeKey: 'rescue.echo.2', kind: 'narration', speakerKey: null, text: '裂隙仍在海面呼吸，提醒每个人获救与安全从来不是同一件事。', order: 0 },
    { beatKey: 'beat.seal.echo.1', nodeKey: 'seal.echo.1', kind: 'narration', speakerKey: null, text: '港城的灯沿城墙逐盏亮起，人群终于停止向内港撤离。', order: 0 },
    { beatKey: 'beat.seal.echo.2', nodeKey: 'seal.echo.2', kind: 'narration', speakerKey: null, text: '海面没有传回汽笛，守钟人把未能归航的名字写进值夜簿。', order: 0 },
  )
  const rescueChoice = baseRuntime.narrative.choices.find(choice => choice.choiceKey === 'choice.rescue')!
  const sealChoice = baseRuntime.narrative.choices.find(choice => choice.choiceKey === 'choice.seal')!
  rescueChoice.targetNodeKey = 'rescue.echo.1'
  sealChoice.targetNodeKey = 'seal.echo.1'
  baseRuntime.narrative.choices.push(
    ...
    { choiceKey: 'choice.rescue.echo.1', sourceNodeKey: 'rescue.echo.1', text: '确认获救者已经靠岸', description: '听完归航者的回应。', unavailableReason: '', targetNodeKey: 'rescue.echo.2', displayConditionJson: '{}', availableConditionJson: '{}', effectsJson: '[]', tags: [], order: 0 },
    { choiceKey: 'choice.rescue.echo.2', sourceNodeKey: 'rescue.echo.2', text: '承担裂隙仍存的后果', description: '把未解决的风险带入结局。', unavailableReason: '', targetNodeKey: 'ending.rescue', displayConditionJson: '{}', availableConditionJson: '{}', effectsJson: '[]', tags: [], order: 0 },
    { choiceKey: 'choice.seal.echo.1', sourceNodeKey: 'seal.echo.1', text: '确认港城屏障已经稳定', description: '听见城墙内的人群回应。', unavailableReason: '', targetNodeKey: 'seal.echo.2', displayConditionJson: '{}', availableConditionJson: '{}', effectsJson: '[]', tags: [], order: 0 },
    { choiceKey: 'choice.seal.echo.2', sourceNodeKey: 'seal.echo.2', text: '承担船队失去灯火的后果', description: '把未归者的命运带入结局。', unavailableReason: '', targetNodeKey: 'ending.seal', displayConditionJson: '{}', availableConditionJson: '{}', effectsJson: '[]', tags: [], order: 0 },
  )
  ...
  for (const ending of baseRuntime.narrative.nodes.filter(node => node.kind === 'ending')) {
    ...
    baseRuntime.narrative.beats.push({
      beatKey: `beat.${ending.key}.keeper`, nodeKey: ending.key, kind: 'dialogue',
      speakerKey: profile.characterKey,
      text: ending.key === 'ending.rescue'
        ? '守钟人说：“船回来了，但裂隙还在。明天起，我们一起偿还今晚的选择。”'
        : '守钟人说：“城墙守住了。我会替你记住海面上那段没有回应的黑暗。”',
      order: 1,
    })
  }
  ...
  const coreScene = baseRuntime.adventure!.scenes.find(scene => scene.key === 'scene.core')!
  const decisionOptions = [
    { choiceKey: 'choice.promise-people', conditionKey: 'condition.promise-people', sourceNodeKey: 'stance', echoNodeKeys: ['crossroads', 'rescue.echo.1', 'seal.echo.1'] },
    { choiceKey: 'choice.promise-city', conditionKey: 'condition.promise-city', sourceNodeKey: 'stance', echoNodeKeys: ['crossroads', 'rescue.echo.1', 'seal.echo.1'] },
    { choiceKey: 'choice.rescue', conditionKey: 'condition.decision-rescue', sourceNodeKey: 'crossroads', echoNodeKeys: ['rescue.echo.1', 'rescue.echo.2'] },
    { choiceKey: 'choice.seal', conditionKey: 'condition.decision-seal', sourceNodeKey: 'crossroads', echoNodeKeys: ['seal.echo.1', 'seal.echo.2'] },
  ]
  for (const option of decisionOptions) {
    baseRuntime.adventure!.conditions.push({
      key: option.conditionKey, title: `已作出决定：${option.choiceKey}`,
      description: '技术夹具中由正式叙事行动写入、供后续节点观察的持久选择。', tags: ['decision'],
    })
    const choiceActionKey = `action.${option.choiceKey}`
    const decisionChoice = baseRuntime.narrative.choices.find(choice => choice.choiceKey === option.choiceKey)
    if (!decisionChoice) {
    ...
    baseRuntime.adventure!.actions.push({
      ...
      requirements: [{ narrativePath: '__storyforge.currentNarrativeNodeKey', narrativeEquals: option.sourceNodeKey }],
      rule: { kind: 'automatic' },
      successEffects: [{ op: 'apply-condition', conditionKey: option.conditionKey, duration: null }],
      costlySuccessEffects: [], failureEffects: [], successText: '你的决定已经改变后续人物与场景的反应。',
      costlySuccessText: '你付出代价后坚持了决定。', failureText: '这个决定没有生效。',
      unavailableText: '当前叙事节点不能作出这个决定。', repeatable: false,
      narrativeChoiceKey: option.choiceKey, interaction: null,
    })
    coreScene.actionKeys.push(choiceActionKey)
    for (const echoNodeKey of option.echoNodeKeys) {
      const echoActionKey = `action.echo.${option.choiceKey}.${echoNodeKey}`
      baseRuntime.adventure!.actions.push({
        key: echoActionKey, kind: 'look', label: `回响：${option.choiceKey}`,
        description: '只在相应决定已经发生且进入后续节点时显示。',
        locationKey: 'location.core', targetKey: null,
        requirements: [
          { narrativePath: '__storyforge.currentNarrativeNodeKey', narrativeEquals: echoNodeKey },
          { conditionKey: option.conditionKey, conditionPresent: true },
        ],
        rule: { kind: 'automatic' }, successEffects: [], costlySuccessEffects: [], failureEffects: [],
        successText: `你先前的决定“${option.choiceKey}”正在改变眼前人物的态度和可用道路。`,
        costlySuccessText: '这项选择的代价再次显现。', failureText: '这段回响没有发生。',
        unavailableText: '只有对应选择发生后才能观察这段回响。', repeatable: true,
        narrativeChoiceKey: null, interaction: null,
      })
