...
} from '../../../src/lib/workspace/ownership'
...
async function stripC1Roots(projectId: number) {
  ...
  await db.productQualityGateReceipts.where('projectId').equals(projectId).delete()
  await db.productBuildArtifacts.where('projectId').equals(projectId).delete()
  await db.productBuilds.where('projectId').equals(projectId).delete()
  await db.productProductionCommands.where('projectId').equals(projectId).delete()
  await db.productProductionBriefs.where('projectId').equals(projectId).delete()
  await db.productProductions.where('projectId').equals(projectId).delete()
  ...
  await db.productReleases.where('projectId').equals(projectId).delete()
  ...
  await db.ttrpgRulePacks.where('projectId').equals(projectId).delete()
  for (const spec of PROJECT_TABLES) {
    const locator = spec.domainOwner?.locator
    const isLegacyStamped = locator?.kind !== 'parent'
      && (spec.domainOwner?.defaultOwner === 'world' || spec.domainOwner?.defaultOwner === 'work')
  ...
  await db.productRuntimeSessions.where('projectId').equals(projectId).modify(session => {
    delete session.worldId
    delete session.workId
    delete session.worldReleaseId
    delete session.productReleaseId
...
describe('WORLD-2C C2 · lazy workspace ownership migration', () => {
  ...
  it('历史工作区存在半套 World/Work 根时失败关闭，不猜测归属', async () => {
    const { projectId } = await createLegacyProject('半成品历史归属')
    await db.worlds.add({
      projectId,
      code: 'partial-owner',
      name: '孤立世界',
    ...
    await expect(ensureWorkspaceOwnership(projectId))
      .rejects.toThrow('旧工作区已有未登记的 World/Work 根')
    expect(await db.ownershipMigrations.where('projectId').equals(projectId).count()).toBe(0)
  })
  ...
  it('完整旧项目迁移为一个默认 World/Work，保留业务主键、正文和引用', async () => {
    ...
    expect(await db.productRuntimeSessions.get(seeded.runtimeParent)).not.toHaveProperty('workId')
  ...
  it('采纳 active 指针明确的 C1 根，并以 World 身份作为唯一权威', async () => {
