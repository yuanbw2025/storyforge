...
function databaseName(): string {
  const name = `legacy-v56-character-interaction-${Math.random()}`
  names.push(name)
  return name
}
...
class LegacyCharacterInteractionV56DB extends Dexie {
  constructor(name: string) {
    super(name)
    this.version(55).stores({
      gameDefinitions: '++id, projectId, worldId, workId, &[workId+productKey], productType, status, narrativeModuleId, updatedAt',
...
describe('LEGACY-MIGRATION · v55 -> v56 character-interaction precursor', () => {
  it('只新增空作者内容表，保留既有 GameDefinition 且稳定 key 唯一', async () => {
    ...
    const upgraded = track(new LegacyCharacterInteractionV56DB(name))
    await upgraded.open()
    expect(await upgraded.table('gameDefinitions').get(definitionId)).toMatchObject({ productKey: 'legacy-chat' })
