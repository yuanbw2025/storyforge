...
function databaseName(): string {
  const name = `legacy-v54-game-release-${Math.random()}`
  names.push(name)
  return name
}
...
class LegacyProductReleaseV54DB extends Dexie {
  constructor(name: string) {
    super(name)
    this.version(53).stores({
      simulationSessions: '++id, projectId, worldGroupId, worldId, workId, worldReleaseId, narrativeModuleId, kind, status, parentSessionId, updatedAt',
    ...
    this.version(54).stores({
      ...
      productReleases: '++id, projectId, worldId, workId, gameDefinitionId, worldReleaseId, &[gameDefinitionId+version], contentHash, createdAt',
      ...
      simulationSessions: '++id, projectId, worldGroupId, worldId, workId, worldReleaseId, productReleaseId, narrativeModuleId, kind, status, parentSessionId, updatedAt',
...
describe('LEGACY-MIGRATION · v53 -> v54 game-release precursor', () => {
  it('只新增空内容/发布表并保留旧 SIM 会话', async () => {
    ...
    const upgraded = track(new LegacyProductReleaseV54DB(name))
    await upgraded.open()
    ...
    expect(await upgraded.table('productReleases').count()).toBe(0)
    expect(await upgraded.table('narrativeBeats').count()).toBe(0)
    expect(await upgraded.table('narrativeChoices').count()).toBe(0)

    const productReleaseId = await upgraded.table('productReleases').add({
      projectId: 1,
      worldId: 2,
      workId: 3,
      gameDefinitionId: 8,
    ...
    await upgraded.table('simulationSessions').update(sessionId, { productReleaseId })
    expect(await upgraded.table('simulationSessions').where('productReleaseId').equals(productReleaseId).count()).toBe(1)
  })

  it('新内容稳定 key 的唯一索引拒绝同模块重复记录', async () => {
    const database = track(new LegacyProductReleaseV54DB(databaseName()))
    await database.open()
    const beat = {
      projectId: 1,
      moduleId: 2,
    ...
    await database.table('productReleases').add(release)
    await expect(database.table('productReleases').add({
      ...release,
