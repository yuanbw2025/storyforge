...
function worldRow(page: Page, name: string) {
  return page.locator('.lf-library-grid article').filter({
...
test('Creator正式游戏经UI篡改拒绝、导出导入、玩家恢复与项目删除保持完整', async ({ page }) => {
  ...
  await page.goto(`./home/data?project=${seeded.projectId}`)
  await expect(page.getByRole('heading', { name: '数据与备份', exact: true, level: 2 })).toBeVisible({ timeout: 20_000 })
  ...
  await expect.poll(() => Number(new URL(page.url()).searchParams.get('project')), {
    timeout: 30_000,
  }).not.toBe(seeded.projectId)
  await expect(page).toHaveURL(/\/storyforge\/home\/data\?project=\d+$/)
  const importedProjectId = Number(new URL(page.url()).searchParams.get('project'))
  ...
  const imported = await page.evaluate(async ({ importedProjectId, productionKey }) => {
    ...
    if (!project.activeWorldId) throw new Error('浏览器 Creator 导入缺少活动世界')
    await db.worlds.update(project.activeWorldId, { name: importedName, updatedAt: Date.now() })
    return {
  ...
  await page.goto(`./openworld/runtime?project=${imported.projectId}&mode=play`)
  ...
  const importedRow = worldRow(page, imported.importedName)
  await expect(importedRow).toHaveCount(1)
  await importedRow.getByRole('button', { name: '删除世界', exact: true }).click()
  ...
  await expect(worldRow(page, imported.importedName)).toHaveCount(0)
  await expect(worldRow(page, seeded.projectName)).toHaveCount(1)
