  ...
  const overflow = await page.evaluate(() => ({
    ...
    elements: Array.from(document.querySelectorAll<HTMLElement>('body *'))
      .filter(element => {
        ...
        // The shared product header is an intentional horizontal navigation
        // rail on narrow screens; its off-screen links stay clipped by the
        // scroll container and do not widen the document.
        if (element.closest('nav[aria-label="产品导航"]')) return false
