...
async function openCreatorProduction(page: Page) {
  await page.reload()
  ...
  await expect(page.getByRole('heading', { name: '文字开放世界制作', exact: true }))
...
test('Creator 受治理内容浏览器在桌面与窄屏覆盖内容、Artifact、诊断、搜索和分页', async ({ page }) => {
  ...
  const browser = await openCreatorProduction(page)
