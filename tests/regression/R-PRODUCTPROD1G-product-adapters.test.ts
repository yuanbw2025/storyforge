...
describe('R-PRODUCTPROD-1G · upper-product adapter registry', () => {
  it('文字开放世界不再经过通用节点 adapter，而由专属 Creator 生产链负责', () => {
  ...
  it('现行上层产品共享 registry，但未过 Golden 的 TTRPG 不得标商业就绪', () => {
    ...
    expect(catalog.map(item => item.productType)).toEqual(['character-interaction', 'ai-town', 'text-adventure', 'avg', 'ttrpg'])
    expect(catalog.filter(item => item.commercialReady).map(item => item.productType)).toEqual(['character-interaction', 'ai-town', 'text-adventure', 'avg'])
...
describe('R-PRODUCTPROD-1H · current generic non-TTRPG playable Build Previews', () => {
  beforeEach(async () => { await db.delete(); await db.open() })
  afterAll(() => db.close())

  it('通用产品均从不可变 Build Preview 建立正式会话，并初始化各自玩法状态', async () => {
    const owned = await workspace('通用产品预览闭环')
    const revision = await createWorldRevision({ scope: owned.scope, label: '通用产品冻结来源' })
