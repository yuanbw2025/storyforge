...
describe('Text Open World vNext · three registries and complete data lifecycle', () => {
  ...
  it('openWorldRuntime只装配vNext玩家可见状态、Action闭集和正式结果', async () => {
    ...
    expect(context.text).toContain('【已学技能】')
  ...
  it('PROJECT_TABLES派生的备份恢复会重映射完整ProductRelease、Session、Event与Checkpoint链', async () => {
    ...
    expect(backup.productRuntimeCheckpoints[0]).toMatchObject({ purpose: 'manual', subjectKey: null })
    ...
    expect(importedCheckpoint).toMatchObject({ sessionId: importedSession?.id, purpose: 'manual', subjectKey: null })
