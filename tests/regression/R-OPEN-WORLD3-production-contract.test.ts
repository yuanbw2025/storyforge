...
describe('R-OPEN-WORLD3 · product production contract and P0-P10 DAG', () => {
  ...
  it('生成共享Harness可接受的多Skill有界计划，并把所有任务汇入发布QA', async () => {
    ...
    expect(TEXT_OPEN_WORLD_PRODUCTION_TOKEN_BUDGET_WEIGHT_V1).toBe(202)
    ...
    expect(plan.tasks.find(task => task.taskKey === 'p5.mainline')?.budgetReservation.inputTokens)
      .toBe(Math.floor(parsedBrief.productionBudget.maximumInputTokens * 10 / 202))
    expect(plan.tasks.find(task => task.taskKey === 'p8.catalog.progression')?.budgetReservation.inputTokens)
      .toBe(Math.floor(parsedBrief.productionBudget.maximumInputTokens * 6 / 202))
    expect(plan.tasks.find(task => task.taskKey === 'p8f.quest-finalize')?.budgetReservation.inputTokens)
      .toBe(Math.floor(parsedBrief.productionBudget.maximumInputTokens * 19 / 202))
    expect(plan.tasks.find(task => task.taskKey === 'p9.scene-scripts')?.budgetReservation.inputTokens)
      .toBe(Math.floor(parsedBrief.productionBudget.maximumInputTokens * 26 / 202))
