...
import { CURRENT_PRODUCT_SOURCE_CATALOG } from '../helpers/current-product-world'
import { createTextAdventureFoundationRuntimePackageV2 } from '../helpers/text-adventure-v2-foundation'
import { createCurrentProductBriefFixture } from '../helpers/current-runtime-package'
...
async function mainRouteReceipt(runtime: ProductRuntimePackageV1, packageHash: string): Promise<ProductQualityGateReceiptV1> {
...
async function fixture() {
  ...
  for (const ending of baseRuntime.narrative.nodes.filter(node => node.kind === 'ending')) {
    baseRuntime.narrative.beats.find(beat => beat.nodeKey === ending.key)!.text += `此后的许多个清晨，人们仍会从这次抉择谈起：有人记住获救者的名字，也有人记住你没有逃避代价。${ending.title}不是一句胜利宣言，而是一份必须继续履行的责任。`.repeat(2)
  }
  baseRuntime.adventure!.actions.find(action => action.key === 'action.look.harbor')!.successText += '你逐行核对潮汐图上的旧标记、守灯人的笔记与历次风暴留下的航线变化。'.repeat(110)
  ...
  const runtime = (await createFixtureProductReleaseManifestV1({
    runtimePackage: authoredRuntime,
  })).runtimePackage
  ...
  expect(calculatedQuality, JSON.stringify(calculatedQuality, null, 2)).toMatchObject({ passed: true })
  ...
  expect(autoplay.passed, JSON.stringify(autoplay, null, 2)).toBe(true)
