...
describe('PRODUCT-PROD-1B · frozen source selector UI', () => {
  ...
  it('作者选择角色起点后可增删冻结素材，Brief 摘要反映 exact 子集', async () => {
    ...
    await act(async () => {
      root.render(createElement(ProductProductionStudio, {
        scope: owned.scope, worldGroupId: owned.world.worldGroupId,
        initialProduct: 'avg', allowedProducts: ['text-adventure', 'avg', 'text-open-world'],
        initialSource: {
          schema: 'storyforge.product-production-handoff', version: 1, productType: 'avg',
    ...
    const activity = host.querySelector('[data-testid="product-production-command-activity"]')
    ...
    await waitFor(() => {
      const summary = host.querySelector('[data-testid="product-production-source-selection-summary"]')
