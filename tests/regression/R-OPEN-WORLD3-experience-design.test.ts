...
describe('R-OPEN-WORLD3 · V3运行包装配与QA', () => {
  ...
  it('G7-01至G7-06 盐脊完整DAG产出叙事玩法闭环与最低媒资', async () => {
    ...
    const budgetLedger = JSON.parse(build.budgetLedgerJson) as {
      attempts: Array<{ taskKey: string; usage: { modelCalls: number } | null }>
    }
    expect(budgetLedger.attempts
      .filter(attempt => attempt.taskKey === 'p1.source-curation')
      .reduce((sum, attempt) => sum + (attempt.usage?.modelCalls ?? 0), 0)).toBe(p1ModelAttempts)
    ...
    for (const attempt of importedLedger.attempts as Array<{ runId: number; attempt: number }>) {
      expect(importedRunIds.has(attempt.runId)).toBe(true)
