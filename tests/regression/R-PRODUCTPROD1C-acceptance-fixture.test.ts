...
import {
  textOpenWorldProductArtifactStoreV1,
...
describe('R-PRODUCTPROD-1C · authorized deterministic acceptance fixture', () => {
  ...
  it('暂停会递增 controlEpoch，旧执行者不能再接受任何 Artifact', async () => {
    ...
    await expect(textOpenWorldProductArtifactStoreV1.acceptProductBuildArtifact({
  ...
  it('已封存 Build 可用可移植 seal 向直接后继 Build 复用精确 Artifact', async () => {
    ...
    const carried = await textOpenWorldProductArtifactStoreV1.carryForwardProductBuildArtifactsAcrossBuildsV1({
      scope: owned.scope,
      sourceBuildId: owned.buildId,
  ...
  it('目标 Plan task.reuse=null 时 direct cross-build carry 拒绝且目标零写入', async () => {
    ...
    await expect(textOpenWorldProductArtifactStoreV1.carryForwardProductBuildArtifactsAcrossBuildsV1({
      scope: owned.scope,
      sourceBuildId: owned.buildId,
      targetBuildId: target.targetBuildId,
      targetControlEpoch: target.targetControlEpoch,
