...
describe('Text Open World vNext · ProductRuntime vNext player UI', () => {
  ...
  it.sequential('正式ProductRelease可从玩家入口启动、执行Action并建立可恢复分支', async () => {
    ...
    const characterPanel = host.querySelector('[data-testid="text-open-world-character-panel"]')
    expect(characterPanel?.textContent).toContain('等级 1 / 20')
    expect(characterPanel?.textContent).toContain('累计 0 EXP')
    expect(characterPanel?.textContent).toContain('攻击6')
    expect(characterPanel?.textContent).toContain('基础生命')
    expect(characterPanel?.textContent).not.toContain('baseHealth:20')
