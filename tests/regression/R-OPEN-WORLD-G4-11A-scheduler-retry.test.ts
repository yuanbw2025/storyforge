...
describe('R-OPEN-WORLD-G4-11A · P9 child-step scheduler recovery', () => {
  ...
  it('晚期子步骤失败后只预留剩余预算，并在同一Run累计真实usage与比例费用上界', async () => {
    ...
    if (ledger.version === 2) {
      const charges = Object.values(ledger.charges ?? {})
      expect(charges).toHaveLength(2)
      expect(new Set(charges.map(charge => charge.attempt))).toEqual(new Set([1, 2]))
      expect(charges.reduce((sum, charge) => sum + charge.usage.modelCalls, 0)).toBe(129)
      expect(charges.reduce((sum, charge) => sum + (charge.costUpperBoundUsd ?? 0), 0)).toBeCloseTo(12.9)
      expect(charges.reduce((sum, charge) => sum + charge.usage.modelCalls, 0))
        .toBeLessThanOrEqual(input.brief.productionBudget.maximumModelCalls)
      expect(charges.reduce((sum, charge) => sum + (charge.costUpperBoundUsd ?? 0), 0))
        .toBeLessThanOrEqual(input.brief.productionBudget.maximumCostUsd!)
    } else {
  ...
  it('重试已付用量超过剩余任务预算时保留charge但拒绝候选且不再重试', async () => {
    ...
    const ledger = JSON.parse(build.budgetLedgerJson) as {
      ...
      charges?: Record<string, {
        costUpperBoundUsd: number | null
        usage: { modelCalls: number; inputTokens: number }
      }>
    }
    expect(ledger.version).toBe(2)
    const charges = Object.values(ledger.charges ?? {})
    expect(charges).toHaveLength(2)
    expect(charges.reduce((sum, charge) => sum + charge.usage.modelCalls, 0)).toBe(130)
    expect(charges.reduce((sum, charge) => sum + charge.usage.inputTokens, 0)).toBe(130_001)
    expect(charges.reduce((sum, charge) => sum + (charge.costUpperBoundUsd ?? 0), 0)).toBeCloseTo(13)
