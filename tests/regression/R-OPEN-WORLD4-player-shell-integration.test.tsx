...
describe('Text Open World G4 · 真实 vNext 玩家壳集成', () => {
  ...
  it.sequential('新旅程把现有能力分配到五个视图，纯导航不改变运行投影和时间线', async () => {
    ...
    expect(character.querySelector('[data-testid="text-open-world-skills-statuses"]')).toBeTruthy()
    expect(character.textContent).toContain('Lv.1 · 0 EXP')
    expect(character.textContent).toContain('三项属性')
