// Current character-interaction runtime contract; this is not a legacy alias fixture.
...
describe('CHARACTER-INTERACTION-2A · 确定性角色互动内核', () => {
  beforeEach(async () => {
    await db.delete()
    await db.open()
  })
  ...
  async function formalInteractionSession(maxTurns = 200) {
    const owned = await seedCurrentProductWorld('CHARACTER-INTERACTION-2A 正式产品上下文')
    ...
    const runtimePackage: ProductRuntimePackageV1 = {
      ...
      productType: 'character-interaction',
      definition: {
        productKey: 'character-interaction.context.boundary',
        title: '钟楼会面',
      ...
      interaction: {
        ...
        sceneTemplates: [{
          ...
          endingNodeKey: null, maxTurns, directorBudget: 50_000, order: 0,
        }],
      },
    }
  ...
  it('知识只经真实告知传播，关系变化有证据与幅度规则，记忆不能伪造', async () => {
    const created = (await formalInteractionSession()).session
    ...
    let state = await readProductRuntimeState(created.id!)
    ...
    state = await readProductRuntimeState(created.id!)
  ...
  it('过期回复被拒绝，回放、检查点和分支保持独立', async () => {
    ...
    const checkpoint = await createProductRuntimeCheckpoint({ sessionId: created.id!, name: '钟楼检查点' })
    expect(await verifyProductRuntimeCheckpoint(checkpoint.id!)).toBe(true)
    const beforeBranch = await readProductRuntimeState(created.id!, reply.sequence)
    const child = await branchProductRuntimeSession({
      parentSessionId: created.id!,
      throughSequence: reply.sequence,
      title: '另一种回答',
    })
    expect((await readProductRuntimeState(child.id!)).interaction?.messages.map(item => item.text))
      .toEqual(beforeBranch.interaction?.messages.map(item => item.text))
    expect((await readProductRuntimeState(child.id!)).interaction?.messages.every(item => item.eventSequence < 0)).toBe(true)
    await commitInteractionPlayerMessage({
      ...await envelope(child.id!, 'branch.message'),
      messageId: 'branch:message',
      text: '只发生在分支。',
    })
    expect((await readProductRuntimeState(created.id!)).interaction?.messages.some(item => item.messageId === 'branch:message')).toBe(false)
    const childSequences = (await readProductRuntimeState(child.id!)).interaction?.messages.map(item => item.eventSequence) ?? []
    expect(new Set(childSequences).size).toBe(childSequences.length)
  })

  it('100 轮后上下文保持有界且已采用的关键承诺不会被摘要丢弃', async () => {
    const created = (await formalInteractionSession(120)).session
    ...
    const state = await readProductRuntimeState(created.id!)
  ...
  it('CONTEXT_SOURCES 只装配指定角色视角，不泄露其他角色私聊', async () => {
    ...
    const aria = await assembleContext({
      ...
      scope: formal.scope,
      productRuntimeSessionId: created.id!,
