...
async function planFixture() {
  ...
  const plan = await createProductProductionPlanV3({ buildNumber: 1, briefHash, brief: parsedBrief })
...
describe('R-PRODUCTPROD-1C · bounded parallel production plan', () => {
  ...
  it('拒绝错误 Brief hash，避免用另一份授权 Brief 启动 Build', async () => {
    const parsedBrief = brief()
    await expect(createProductProductionPlanV3({
  ...
  it('拒绝循环依赖和多个任务争抢同一 Artifact owner', async () => {
    ...
    expect(() => parseProductProductionPlanV3(cyclic, parsedBrief, briefHash)).toThrow(/DAG 有环/)

    const duplicate = structuredClone(plan)
    duplicate.tasks.find(task => task.taskKey === 'content.product-module')!.outputArtifactKeys = ['design.game']
    expect(() => parseProductProductionPlanV3(duplicate, parsedBrief, briefHash)).toThrow(/多个 owner/)
  })
  ...
  it('拒绝越过用户授权预算和遗漏必需 provider capability', async () => {
    ...
    expect(() => parseProductProductionPlanV3(overBudget, parsedBrief, briefHash)).toThrow(/预算预留超过/)
