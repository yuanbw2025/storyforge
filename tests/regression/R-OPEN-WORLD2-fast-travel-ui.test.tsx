...
describe('Text Open World vNext · fast travel player UI', () => {
  ...
  it('只为已到访且已解锁的目的地显示同一正式快旅Action', async () => {
    ...
    await act(async () => root.render(createElement(TextOpenWorldMapPanel, { projection, runtimeEventSequence: 0, busy: false, sessionKey: 9, onTravel })))
    ...
    const chooseFastTravel = Array.from(host.querySelectorAll('[aria-label="快速旅行列表"] button'))
      .find(button => button.textContent === '查看快旅路线') as HTMLButtonElement | undefined
    expect(chooseFastTravel).toBeTruthy()
    await act(async () => chooseFastTravel!.click())
    expect(onTravel).not.toHaveBeenCalled()
    const fastTravel = host.querySelector('button[aria-label^="快速旅行"]') as HTMLButtonElement | null
    ...
    expect(onTravel).toHaveBeenCalledWith({
      ...
      destinationLocationKey: 'location.salt-port',
    })
  })
